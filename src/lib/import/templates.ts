...
export const IMPORT_TEMPLATES: readonly ImportTemplate[] = [
  {
    ...
    docs: "One product per row. category_slug must match an existing category. Occasions, images and custom-field options take comma or | separated lists; custom field types are SELECT, TEXT, SWATCH, SIZE, NUMBER or FILE. tier is the catalog tier 1-4 (1 owner · 2 resin goods · 3 supplies · 4 3D print) — products without a tier sort last on the shop; in_stock is TRUE/FALSE (defaults to in stock). Empty tier/in_stock cells leave existing values unchanged on update.",
  },
  {
    key: "categories",
    label: "Categories",
