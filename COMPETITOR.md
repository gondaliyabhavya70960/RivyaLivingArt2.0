# COMPETITOR.md — Top-20 Competitor Research

Competitor landscape for **ResinRiva** ([store.bhavyagondaliya.co.in](https://store.bhavyagondaliya.co.in) — luxury made-to-order resin art & custom 3D printing, India, every order finalized on WhatsApp `wa.me/917096036250`). Researched **July 2026 via web search**. Competitor sites are research reference **ONLY** — for product TYPES, category ideas, pricing signals, and blog TOPICS. **Never copy their text, product names, descriptions, images, or photos.**

> **Method note:** This research is **search-snippet-based**. This environment could not fetch competitor pages directly (direct fetches blocked by network policy), so every claim below is derived from search-result titles, snippets, and third-party pages surfaced in web search. Unknowns are explicitly marked — hedges such as "not visible in search results" and "(inferred)" are deliberate and must be preserved; treat them as items to verify live before acting on them.

---

## India — direct competitors

### Sumaiya Resin Arts
- **URL · Country:** https://sumaiyaresin.art/ · India (Tamil Nadu, ships pan-India)
- **Niche / Focus · Product Types · Price Positioning:** Customized handmade resin gifts by a solo homemaker-artist: keychains/keyrings, resin jewellery (earrings, pendants, rings, bangles, bracelets), photo frames, wall clocks, nameplates, baby-memory preservation, varmala preservation, birthday/wedding return gifts (bulk). Price positioning: budget-to-mid, prices disclosed in blog/FAQ content — keychains Rs 40–150, anniversary gifts from Rs 399, photo frames from Rs 799; final price "depends on size and customization". Not a luxury positioning.
- **Ordering Model:** WhatsApp-first: choose a design on the website or on WhatsApp (+91 84894 62381), confirm order details in chat, product photo shared on WhatsApp before dispatch; dispatch in 4–5 days. No shopping cart visible in search results; site offers a "free quote".
- **Strengths:**
  - Strong programmatic local SEO: dedicated "Resin Art in [city]" landing pages ranking for Chennai, Kerala, Delhi, Bangalore, Tamil Nadu
  - Active blog plus Google Web Stories (gift listicles, occasion posts, price-guide posts targeting 2025/2026 keywords)
  - Trust signals in search: Scamadviser trust score 61 (rated legit), TrustProfile listing, "trusted by 1000+ customers" claim
  - Clear WhatsApp workflow with pre-dispatch photo approval and fast 4–5 day dispatch across India
  - Authentic founder story (homemaker-turned-artist, handmade not mass-produced) used as brand differentiator
- **Weaknesses:**
  - Budget craft positioning with entry prices as low as Rs 40 — no premium/luxury tier visible in search results
  - Solo-artist production model limits scale and throughput (inferred from "all products hand-made by Sumaiya")
  - No structured e-commerce catalog visible in search results — pricing lives in blog prose and chat quotes rather than product pages
  - Moderate third-party trust score (61/100 on Scamadviser); no 3D printing, workshops, or big-ticket items (tables, wall panels) visible in search results
- **Content & SEO strategy:** Best content-SEO operator of the India core set: ranks via city landing pages ("resin art in Chennai/Delhi/Kerala/Bangalore"), keyword-dated listicles ("Top 10 Personalized Resin Art Gift Ideas for 2026", "Why Resin Wall Clocks Are the Most Trending in India 2025"), occasion posts ("Happy Anniversary Bhaiya and Bhabhi"), bulk-keychain B2B pages, FAQ page, and Web Stories. Blog is active and price-transparent.
- **What ResinRiva will do better:**
  - Structured catalog with price bands and per-product customization forms feeding WhatsApp gives buyers a self-serve configure-then-chat flow, versus Sumaiya's chat-only quoting where pricing is buried in blog prose
  - Luxury positioning plus the 3D printing product line addresses the premium and geometric-decor segments Sumaiya's Rs 40–799 craft-gift range does not touch
  - Full schema markup, OG cards and sitemap plus portfolio case studies and a workshops page give richer SERP presence and service depth than Sumaiya's blog-only footprint (inferred from search-visible page types)

### Resin Arts Jaipur
- **URL · Country:** https://resinartsjaipur.com/ · India (Jaipur, Rajasthan; export claimed)
- **Niche / Focus · Product Types · Price Positioning:** Premium resin studio and memory-preservation agency: varmala/flower preservation, baby keepsakes, epoxy nameplates, mantra/Navkar frames, wall decor panels, clocks, ring platters, resin jewellery; plus epoxy resin material supply and paid resin art classes/workshops (online and offline). Runs a dedicated sister site varmalapreservation.com. Price positioning: unclear — quote-only. No product prices visible anywhere in search results; snippets explicitly say cost "depends on specific requirements" with detailed quotes before proceeding. Free shipping on wall clocks within Jaipur; same-day Jaipur delivery.
- **Ordering Model:** Enquiry/consultation-based: WhatsApp, email, or in-person office consultations; quotes given after requirements (home visits quoted for wall panels); 24-hour pick-up and drop-off service for preservation items. No cart or listed prices visible in search results.
- **Strengths:**
  - Heavy social proof: claims 15,000+ memory-preservation projects, 950+ satisfied clients, 25+ artists, and a dedicated reviews page advertising 4.8/5
  - Deepest service range of the India core set: preservation, decor, jewellery, material supply, and paid classes/workshops all under one brand
  - Multi-domain SEO footprint (resinartsjaipur.com, resinartsjaipur.in, varmalapreservation.com) with a dedicated preservation brand ranking on its own keywords
  - Founder-led brand (Kavita Dadhich, since 2018) with emotionally strong preservation testimonials surfacing in search
  - Local logistics moat in Jaipur: same-day delivery, home visits, 24-hour pickup for preservation
- **Weaknesses:**
  - Zero price transparency in search results — every product is quote-gated, adding friction versus catalog stores
  - Keyword targeting is intensely Jaipur-local ("resin clock dealer Jaipur", "epoxy clock supplier Jaipur"), capping national organic reach
  - Pages read as local-business service pages rather than a structured product catalog (inferred from search-visible URL/title patterns)
  - Brand-confusion exposure: near-identical competitor domains resinartjaipur.com and resinartsjaipur.in appear alongside it in search results
- **Content & SEO strategy:** Ranks for Jaipur-intent commercial keywords across many service pages (classes, wall decor, ring platters, mantra frames, material shop); has a blog ("Resin Home Decor and Interior Guide from Experts") and a dedicated reviews page; sister site varmalapreservation.com carries its own shop (54+ paginated shop pages) and reviews page. Content is service-page-led rather than editorial-led.
- **What ResinRiva will do better:**
  - Transparent price bands in a structured catalog remove the quote-gate friction that Resin Arts Jaipur imposes on every purchase — high-intent buyers can self-qualify before the WhatsApp conversation
  - Full schema markup, OG cards and sitemap plus a fast luxury UI position ResinRiva for national SERPs, versus their Jaipur-saturated local-dealer keyword strategy
  - ResinRiva also runs a workshops page and a 50+ post blog engine, matching their classes offer while adding the editorial content layer their service-page-heavy sites lack

### Kanha Kreation
- **URL · Country:** https://kanhakreation.com/ · India (exact base not visible in search results; a related-business address in Jaipur appears in snippets — unconfirmed)
- **Niche / Focus · Product Types · Price Positioning:** Online gift shop mixing handcrafted resin art (trays, coasters, bookmarks, pooja thalis, nameplates, clocks, servers, print frames, wall decor, keychains) with scented candles, silicone molds, and curated gift hampers; occasion gifting for birthdays, weddings, festivals. Price positioning: mid-range, positioned as "luxurious look, affordable price" — Flower Preservation Resin Tray Rs 3,999, Bloom Resin Tray Set Rs 3,500, Resin Pooja Thali 10-inch Rs 2,000–2,200. Fuller price ladder not visible in search results.
- **Ordering Model:** Cart-based e-commerce: /product/ pages with fixed prices and variant pricing, "secure payments, dependable delivery" messaging (WooCommerce-style store, inferred from URL structure); customization by choosing names, initials, colours, themes, charms or prints. No WhatsApp-first flow visible in search results.
- **Strengths:**
  - Structured product pages with visible fixed prices and variants — lower purchase friction than quote-based rivals
  - Diversified gifting catalog (resin plus candles, molds, hampers) captures multi-category gift buyers and festive occasions
  - Distinct traditional/devotional niche in resin pooja thalis with dedicated category pages
  - Publishes SEO landing content targeting commercial keywords ("Best Resin Art Websites in India", "Resin Art Gift Online")
- **Weaknesses:**
  - Very thin index footprint: a site: query surfaces only a handful of pages, suggesting weak crawl depth or a small/young site
  - Severe brand-name collision: kanhacreation.com (different store), multiple unrelated "Kanha Creations" Instagram accounts and Facebook pages dilute branded search
  - No customer reviews, trust badges, founder story, or press visible in search results
  - No workshops, preservation services, or about/location transparency visible in search results
- **Content & SEO strategy:** Minimal: a few keyword landing articles ("resin art websites in India", "find exceptional resin art gift online") plus product-category archives. No dated blog cadence, web stories, or city pages visible in search results; branded SERP is polluted by similarly named businesses.
- **What ResinRiva will do better:**
  - ResinRiva's 50+ post blog engine and portfolio case studies dwarf Kanha Kreation's handful of indexed landing pages, compounding organic reach over time
  - Per-product customization forms that feed directly into WhatsApp give a concierge made-to-order experience versus their generic add-to-cart with limited personalization fields
  - Full schema markup, OG cards and sitemap plus a distinctive luxury brand identity avoid the crawl-thinness and name-collision problems visible in Kanha Kreation's search presence

### SAASHI
- **URL · Country:** https://saashi.co.in/ · India (Pune, Maharashtra; free shipping claimed pan-India)
- **Niche / Focus · Product Types · Price Positioning:** Handmade resin art shop and self-described "manufacturer & supplier": geode and ocean wall art, resin tables and marble table tops, serving/puja platters, trays, coasters, wall clocks, nameplates, mantra frames, photo frames, keychains, jewellery, rakhis; custom orders page for bespoke pieces. Price positioning: budget-to-mid with aggressive strikethrough discounting — keychains Rs 245–249 (from Rs 299–399), jewellery Rs 199–399, coaster sets Rs 799–999 (from Rs 1,800), clocks Rs 2,299 (from Rs 2,899), geode wall art Rs 4,000 (from Rs 6,000), tables Rs 3,000–7,500. "Affordable" is used in its own shop-page title.
- **Ordering Model:** Cart-based online store with fixed and sale prices on product pages, plus a dedicated custom-order page for bespoke work; review snippets mention timely delivery. WhatsApp-first flow not visible in search results.
- **Strengths:**
  - Fully structured, price-transparent catalog across many categories including big-ticket items (tables, large wall art) the gift-focused rivals lack
  - Dedicated custom-order page and "manufacturer & supplier" positioning that can capture B2B/bulk demand
  - Positive review snippets in search: creativity, good finishing, nominal prices, quick turnaround on customized pieces
  - Broad keyword landing-page coverage: resin art shop Pune, epoxy resin wall art, resin ocean art, resin home decor products, resin photo frame
- **Weaknesses:**
  - Discount-led merchandising (50%-off strikethroughs on most items) undercuts any premium perception and trains buyers to expect sales
  - No editorial blog cadence visible in search results — content is category/landing-page style, so no top-of-funnel capture
  - No workshops, memory/varmala preservation, or occasion storytelling visible in search results, missing the highest-emotion (and highest-margin) segment
  - Identity is Pune-anchored in titles and positioning, limiting other-city local-intent capture
- **Content & SEO strategy:** Ranks through commercial category landing pages ("Best Resin Art, Resin Geode Art Shop in Pune", "Buy Resin Home Decor Products Online", "Resin Ocean Art", "Epoxy Resin Wall Art") and product pages; multiple homepage variants ("Homepage 2") suggest an unpolished WordPress build (inferred). No blog posts, web stories, or city-series pages visible in search results.
- **What ResinRiva will do better:**
  - ResinRiva holds luxury positioning with clean price bands instead of SAASHI's discount-stacked pricing, protecting margin and brand perception in the same made-to-order categories
  - The 50+ post blog engine, portfolio case studies and workshops page build top-of-funnel and trust layers SAASHI entirely lacks in search
  - Per-product customization forms feeding WhatsApp deliver a guided bespoke flow, versus SAASHI's plain cart plus a single generic custom-order page

### Leoberry Gifts
- **URL · Country:** https://leoberrygifts.com · India (Chennai, Kilpauk; also claims Bangalore corporate gifting presence)
- **Niche / Focus · Product Types · Price Positioning:** Customized gifting generalist spanning resin and 3D printing: customised resin photo frames (4in–10in, some with lights), resin coasters and trinket trays, flower/varmala preservation, 3D printed gifts, birthday/bridesmaid/wedding-return gifts, corporate gifting and employee welcome kits. Price positioning: budget-to-mid, evidence-based — coasters Rs 270–410 (Heavenly Ocean Rs 270, Yellow Hexagon Rs 290, Luxe Rs 350, Glittery Flower Rs 410, Real Rose set of 2 Rs 365); Customised Resin Frame from Rs 420; Wooden Resin Frame Rs 3,899 discounted to Rs 3,199; blog pitches wedding return gifts at Rs 500–2,000.
- **Ordering Model:** Cart (WooCommerce-style product pages and product-category URLs with fixed prices) + WhatsApp widget for customization and bulk (snippets mention "WhatsApp for discounts on 5+ units"); email (leoberrygifts@gmail.com) for order follow-ups.
- **Strengths:**
  - Only researched India competitor covering both resin AND 3D printed gifts — direct category overlap with ResinRiva
  - Structured catalog with visible per-product prices, plus occasion/audience landing pages (bridesmaid, gifts-for-her, gifts-for-parents, office specials)
  - Chennai local SEO: ranks for "wedding return gifts Chennai", "corporate gifts Chennai", "birthday gifts Chennai" style queries with dedicated blog posts
  - Third-party trust surfaces: WedMeGood vendor profile with positive review (100-hamper order), Justdial listing, active Facebook page
  - Pan-India shipping with protective packaging; stated 3–7 day resin curing timeline sets expectations well
- **Weaknesses:**
  - Budget price anchoring (Rs 270 coasters, "every BUDGET" blog framing) undercuts any luxury perception
  - Inconsistent URL architecture visible in search results (leoberrygifts.com/product-category/... vs www.leoberrygifts.com/pages/shop) suggesting migration/SEO hygiene issues; contact page surfaced as "Bot Verification" in the index
  - Gifting-generalist branding rather than art/design-led luxury; no workshops or portfolio storytelling visible in search results
  - Review volume/aggregate rating not visible in search results
- **Content & SEO strategy:** Occasion- and city-keyword strategy: blog posts like "Get Beautiful Wedding Return Gifts Chennai for every BUDGET", "Exclusive Corporate Gifts in Chennai", "All About Customised Resin Frames", plus many category landing pages. Ranks for Chennai gifting queries and its own brand terms; blog depth appears modest (a handful of posts visible in search results).
- **What ResinRiva will do better:**
  - Luxury positioning with price bands avoids Leoberry's low fixed-price anchoring (Rs 270–420) while still giving buyers pricing confidence
  - Per-product customization forms that feed WhatsApp beat Leoberry's generic floating WhatsApp widget — the enquiry arrives pre-structured
  - Clean single-domain architecture with full schema markup, OG cards and sitemap vs Leoberry's mixed www/pages URL mess and bot-verification indexing artifacts; ResinRiva's 50+ post blog engine and workshops page also cover surfaces Leoberry lacks

### Resinart.in (Resin Art Shop)
- **URL · Country:** https://resinart.in · India (Lucknow association per its own city blog post)
- **Niche / Focus · Product Types · Price Positioning:** Customized resin art catalog-breadth play: keychains, coasters, trays, clocks, tables, jewelry, earrings, rakhis, fridge magnets, memory preservation, varmala preservation (teakwood/pinewood frames, add kaleera/wedding card/choora), baby footprint preservation, soy-wax scented candles. Price positioning: unclear — no prices for this domain visible in search results (comparable varmala preservation services elsewhere start Rs 2,999–4,999).
- **Ordering Model:** Enquiry-leaning: a "Contact Us | Buy Epoxy Resin Art Products" page is what surfaces for purchase intent; no cart/checkout pages visible in search results (inferred enquiry/contact-led); /f/ blog URL pattern suggests a low-code website builder.
- **Strengths:**
  - Very wide product-keyword coverage in title/meta ("resin keychains, coasters, trays, clocks, tables, jewelry, rakhis, fridge magnets...")
  - Dedicated varmala preservation landing page targeting the hottest commercial resin keyword cluster in India
  - Head-term title-tag targeting: "Resin Art Shop | Best Resin Art Store in India"
  - Eco/differentiation angle (soy wax candles, lifelong color/shape retention claim for varmala)
- **Weaknesses:**
  - Extremely thin indexed footprint — a site: query surfaced only ~4 pages (home, varmala page, 2 blog posts)
  - No visible pricing anywhere in search results; no reviews or third-party trust signals found
  - Blog is minimal (2 posts visible: a resin gifts guide and a Lucknow city post) — no sustained content engine
  - No structured product pages surfacing for individual SKUs despite the claimed range (inferred: catalog is largely non-indexed or non-existent)
- **Content & SEO strategy:** Head-keyword title tags plus one city page ("Resin art store Lucknow") and one evergreen guide ("The art and craft of resin"); essentially no ranking depth beyond brand and a couple of generic terms. No schema/rich results visible in search results. Ranks #1 for "resin art store India" per discovery-sweep observation.
- **What ResinRiva will do better:**
  - ResinRiva's structured catalog with price bands + per-product customization forms converts search traffic Resinart.in can only funnel to a contact page
  - 50+ post blog engine vs their ~2 posts — ResinRiva can out-publish them on every keyword they touch (varmala, coasters, clocks, preservation)
  - Full schema markup, OG cards and sitemap give ResinRiva rich-result and social-share advantages over a thin site-builder deployment

### Woodensure
- **URL · Country:** https://www.woodensure.com · India
- **Niche / Focus · Product Types · Price Positioning:** Epoxy resin furniture manufacturer-retailer: river/live-edge dining tables, coffee/side/console/nesting/center tables, wider wooden furniture range, plus a 3D epoxy resin wall art category (Sakura Tree set of 3, Seashore, Floret, sets of 2) marketed as home decor. Price positioning: mid-to-premium furniture, discount-led framing — example dining table Rs 55,800 (with promo codes); their own content places India epoxy dining tables at Rs 25,000 to Rs 1,00,000+; sitewide "UPTO 70% OFF" / "25% off" / "61% off" messaging on category titles.
- **Ordering Model:** Full e-commerce cart: buy-online item pages with promo codes and discount percentages; made-to-order with stated dispatch within 30 working days; review snippets say the team is "very responsive on WhatsApp and call" for customizations (color, size), so cart + WhatsApp-assisted customization.
- **Strengths:**
  - Strong SEO'd e-commerce architecture: per-item landing pages, category pages for every table type and for 3D resin wall art, all with buy-intent titles
  - Active blog with fresh, dated content ("10 Best Wedding Gift Ideas 2025-26", "10+ Modern Wall Decor Ideas (2026)", premium epoxy table buyer's guide)
  - Dedicated customer reviews page; snippets show praise for craftsmanship, speed, price and WhatsApp responsiveness
  - Customization positioning ("focuses mainly on customisation") across seater configurations, colors, sizes
  - Owns high-AOV keywords ("epoxy resin dining table India", "3D resin wall art") that adjacent gifting brands don't touch
- **Weaknesses:**
  - Aggressive discount messaging (UPTO 70% OFF in title tags) reads bargain-furniture, not luxury atelier
  - Blog equity is split: some posts live on woodensure.odoo.com subdomain instead of the main domain
  - Crawl hygiene issues visible: a 404 page is indexed in search results
  - No small-format gifting, keepsake/preservation products, or workshops visible in search results — furniture-only funnel with 30-working-day lead times
- **Content & SEO strategy:** Commercial category/item pages ranking for epoxy furniture terms + listicle blog targeting gifting and wall-decor keywords; buyer-guide content ("What makes a premium epoxy resin table different"). Discount-percentage title tags are the signature pattern. Part of content sits on an Odoo subdomain, diluting authority.
- **What ResinRiva will do better:**
  - ResinRiva's fast luxury UI with price bands preserves premium perception where Woodensure's 70%-off title tags erode it
  - Per-product customization forms feeding WhatsApp formalize what Woodensure handles ad hoc over calls/WhatsApp after cart friction
  - Single-domain 50+ post blog engine plus schema/OG/sitemap vs Woodensure's split Odoo blog and indexed 404s; ResinRiva also monetizes the small-format gifting and workshops demand Woodensure ignores

### Poonam Shah Art
- **URL · Country:** https://www.poonamshahart.com · India (Andheri West, Mumbai; clients across Mumbai, Delhi, Bengaluru, Pune, Hyderabad, Assam)
- **Niche / Focus · Product Types · Price Positioning:** Luxury bespoke resin art studio: crystal glass vases, tea light holders, Jesmonite trays and platter sets, wall-spanning art installations, tabletops, wardrobe panels with camouflaged handles, signature resin furniture made from clients' outgrown children's toys, 3-hour workshops, corporate/architect/designer bulk commissions. Price positioning: unclear — no prices visible in search results; positioning signals are explicitly luxury ("Luxury Decor & Workshops" in Instagram bio, bespoke commissions, crystal-embedded pieces, 7–10 person studio team) (inferred premium).
- **Ordering Model:** Enquiry-only: no product/catalog/price pages surface in search results — only home, about and gallery pages; press coverage says clients find her via Instagram and commissions flow through studio consultations. Known WhatsApp-enquiry model with no structured catalog (consistent with everything visible in search results).
- **Strengths:**
  - Strongest press/authority profile of the researched set: The Better India, Channeliam, LBB (Mumbai and Pune), Local Samosa features — high-quality backlinks and founder-story brand equity
  - Truly differentiated signature products (toy-preservation furniture, crystal/stone-embedded decor, wardrobe panels) that resist commoditization
  - B2B pipeline with corporates, architects, designers and industrialists for exclusive and bulk orders
  - Workshops (3-hour, materials included) featured in LBB roundups — community and secondary revenue channel
  - Instagram-first discovery engine that suits high-ticket bespoke buyers
- **Weaknesses:**
  - Near-zero owned SEO surface: only ~5 pages indexed (home, about, gallery sections); no blog, no product pages, no pricing
  - No structured catalog or ordering flow — every sale requires manual DM/consultation, adding friction and capping volume
  - Brand discovery is dependent on Instagram algorithm and third-party press rather than owned search rankings
  - No visible customer review infrastructure (ratings, testimonials pages) in search results
- **Content & SEO strategy:** Effectively none on-site; she ranks through press articles (The Better India, LBB workshop roundups) and Instagram rather than her own content. Her gallery pages carry the only descriptive copy (tabletops, wardrobe panels).
- **What ResinRiva will do better:**
  - ResinRiva's structured catalog with price bands + per-product customization forms feeding WhatsApp delivers the same bespoke feel with 10x less enquiry friction than pure gallery-and-DM
  - Full schema markup, OG cards, sitemap and a 50+ post blog engine capture the organic search demand (varmala preservation, resin nameplates, resin clocks, workshop searches) that Poonam Shah cedes entirely to press and Instagram
  - Portfolio case studies + a dedicated workshops page replicate her storytelling and workshop channels in an indexable, always-on format

### Pacific Resin Art
- **URL · Country:** https://pacificresinart.in/ · India
- **Niche / Focus · Product Types · Price Positioning:** Handcrafted resin decor: epoxy wall clocks, tables, resin wall art, coasters/trays, name plates, varmala (wedding garland) preservation, custom orders. Price positioning: mid-range — wall clocks from Rs 1,999; personalized/varmala pieces from Rs 2,499; catalog items up to ~Rs 11,999 (from search snippets).
- **Ordering Model:** Shopify cart (/collections/, /products/ URLs) plus WhatsApp numbers (+917737969944 / +919660665565) and email for custom orders.
- **Strengths:**
  - Broad finished-goods catalog spanning clocks, tables, wall art and varmala preservation — closest overlap with ResinRiva's category
  - Accepts custom orders with customer-supplied reference images, colors and sizes; team-guided process
  - Pan-India delivery with stated 3–6 business day handcrafting/processing window
  - Visible price laddering across collections (Rs 1,999 to Rs 11,999) helps conversion
  - SEO-tuned collection titles ("Shop Modern, Luxury, Handmade Wall Clocks In India") ranking for clock queries
- **Weaknesses:**
  - No blog or editorial content visible in search results — content marketing appears absent
  - Custom ordering is unstructured ("share your idea" via WhatsApp/email) rather than guided per-product forms
  - Two domains (pacificresinart.in and pacificresinart.com) appear in results, splitting brand equity (inferred)
  - No workshops or community offering visible in search results
- **Content & SEO strategy:** Ranks via optimized Shopify collection pages for "resin wall clocks India", "resin coasters and trays", "resin wall art"; deep pagination indexed (page 4, page 6); Pinterest profile present; no blog posts visible in search results.
- **What ResinRiva will do better:**
  - ResinRiva's structured catalog with price bands + per-product customization forms that feed pre-filled WhatsApp messages beats Pacific's generic "share your idea over WhatsApp" flow
  - ResinRiva's 50+ post blog engine, full schema markup, OG cards and sitemap give it a content/SEO moat Pacific visibly lacks
  - Workshops page and portfolio case studies add authority and revenue streams Pacific does not show in search results

### Tulsi Resin Store (supplies reference — audience overlap, not a finished-goods rival)
- **URL · Country:** https://tulsiresin.com/ · India (Surat)
- **Niche / Focus · Product Types · Price Positioning:** Resin art SUPPLIES superstore: epoxy resin, silicone moulds (claimed India's largest collection), pigments, kits, candle moulds — raw materials, not finished goods. Price positioning: value/volume — claims "India's lowest price"; free delivery on orders above Rs 5,000; ships to 17+ countries.
- **Ordering Model:** Shopify cart (multi-currency /en-us storefront, judge.me reviews integration); no WhatsApp ordering visible in search results.
- **Strengths:**
  - Claimed 5,000+ products and community of 1 lakh+ artists — dominant SKU breadth in Indian resin supplies
  - Judge.me review infrastructure with positive delivery-speed feedback indexed in search
  - International shipping to 17+ countries (UAE, Oman, USA, UK, Germany, Canada) with stated dispatch SLAs
  - Strong collection-page SEO ranking for both "resin supplies India" and "silicone moulds" queries
  - Trust pages (shipping, delivery/return, about) all indexed — mature store hygiene
- **Weaknesses:**
  - Supplies-only: serves DIY hobbyists, not buyers of finished luxury pieces
  - No blog or educational editorial visible in search results despite a huge hobbyist audience
  - Utilitarian volume-store presentation, no luxury positioning (inferred from snippets)
- **Content & SEO strategy:** Ranks on collection and policy pages ("silicone moulds online at best prices", international shipping, delivery/return); Linktree and Instagram presence; content strategy is catalog-SEO only — no articles visible in search results.
- **What ResinRiva will do better:**
  - ResinRiva sells the finished luxury product, so it is not a direct rival — but Tulsi's 1-lakh hobbyist audience is exactly who ResinRiva's workshops page can convert
  - ResinRiva's blog engine can capture Tulsi's top-of-funnel DIY searchers ("how to" resin queries) and route them to workshops or made-to-order pieces
  - Fast luxury UI and portfolio case studies position ResinRiva as the premium outcome of the craft Tulsi merely supplies

### The 3D Printing Store India
- **URL · Country:** https://the3dprintingstore.in/ · India (Bangalore; Swamy Technosystems Pvt Ltd)
- **Niche / Focus · Product Types · Price Positioning:** Personalized 3D printed gifts: name planks, personalized merchandise, desk items — eco-positioned ("90% eco-friendly bio-compostable plastics"). Price positioning: affordable-to-mid gifts — roughly Rs 400 to Rs 2,000+ per search snippets; free standard dispatch (10 working days) or Rs 100 priority dispatch (3 working days).
- **Ordering Model:** Shopify cart with personalization fields; customer service over WhatsApp chat and email (per about/search snippets) — WhatsApp is support, not the ordering channel.
- **Strengths:**
  - 801 judge.me reviews at 5.0 stars with 90 products carrying customer photos/videos — strongest social proof seen in this sweep
  - Claims 90,000+ customers; also has 111 Justdial reviews for its Bangalore location
  - Eco-friendly bio-compostable plastic positioning differentiates on sustainability
  - Clear dispatch SLAs (free 10-day vs Rs 100 3-day priority) reduce purchase anxiety
- **Weaknesses:**
  - Mass-gifting aesthetic (name planks, cable organizers), not luxury decor positioning
  - No resin or mixed-media offering — single-material catalog
  - No blog or editorial content visible in search results
  - Personalization limited to standard cart text fields rather than rich per-product customization (inferred)
- **Content & SEO strategy:** Ranks for "3D printed gifts India" via homepage and collection pages; judge.me review pages and about-us page indexed; social proof is the visible strategy — no content marketing found in search results.
- **What ResinRiva will do better:**
  - ResinRiva uniquely combines resin art and 3D printing under one luxury brand — no competitor in this sweep spans both
  - Per-product customization forms feeding structured WhatsApp orders give a concierge feel versus T3DPS's standard Shopify personalization fields
  - Schema markup + OG cards + 50-post blog + portfolio case studies let ResinRiva contest hybrid queries ("custom 3D printed decor", "resin vs 3D printed nameplate") that T3DPS's catalog-only SEO cannot

---

## Global / international

### Decords
- **URL · Country:** https://www.decords.com · Estonia/EU (inferred from snippets citing Estonia & Germany customers); produces and ships locally in EU & USA
- **Niche / Focus · Product Types · Price Positioning:** 3D-printed home & wall decor: wall shelves (cloud, corner, wave, mushroom, monster-jaw), wall niches/sculptures, vases (origami, U-shaped), planters, lamp shades, trays, hooks, cosplay masks; also a legacy vinyl wall/window decal line; plus a custom 3D printing service (upload STL/OBJ, or send sketches/photos for paid design help, up to 4 solid colors, custom scaling). PLA plastic, ~35 color options, minimalist/Scandinavian styling. Price positioning: unclear — individual prices not visible in search results; positioned as accessible everyday PLA decor rather than luxury (inferred from "renter-friendly", gift-oriented framing).
- **Ordering Model:** Cart (Shopify-style product pages; snippets show "tax included, shipping calculated at checkout"); custom printing handled via a service product page with file upload, not chat.
- **Strengths:**
  - Very large SKU catalog with aggressively keyword-optimized product titles that rank for long-tail "3D printed X decor" queries
  - Deep customization at product level: ~35 colors, multiple sizes (e.g. 4–10 inch vases), scaling on custom jobs
  - Local production in both EU and USA reduces delivery time (8–16 business days standard, tracked) and damage risk
  - Custom 3D printing service including design-from-sketch/photo (with fees) — captures bespoke demand
  - Renter-friendly positioning (no nails, no paint) that gives products a clear use-case story
- **Weaknesses:**
  - Blog is referenced in product copy but almost invisible in search — thin editorial/SEO content beyond product pages
  - No visible third-party review presence (Trustpilot/Reviews.io not surfacing in search); only on-site testimonial-style quotes
  - Templated, keyword-stuffed product titles suggest volume-listing strategy over brand curation — weak luxury signal
  - No visible portfolio/case studies or workshops; brand story not visible in search results
- **Content & SEO strategy:** Ranks almost entirely on product and collection pages ("3D Printed Decor in EU & USA", per-product long-tail titles). Collections paginate 12+ pages deep, indicating scale-based SEO. A blog with "creative guides and how-tos" is mentioned in snippets but no individual posts surfaced in search — content marketing is a visible gap.
- **What ResinRiva will do better:**
  - ResinRiva's 50+ post blog engine plus full schema markup/OG cards/sitemap out-executes Decords' near-invisible blog on non-product search intent
  - Per-product customization forms feeding directly into WhatsApp give a concierge custom-order flow, versus Decords' generic upload-an-STL service product
  - Portfolio case studies and a workshops page build luxury brand depth and trust that Decords' templated catalog-only presence lacks

### East Side Studio London
- **URL · Country:** https://eastsidestudiolondon.co.uk · UK (founded 2022, East London); prints on demand from studios in UK, Netherlands, Australia and USA; ships worldwide
- **Niche / Focus · Product Types · Price Positioning:** Curated modern wall-art prints from independent artists — graphic art, abstract, photography, comics, Japanese, plus a dedicated 3D/CGI prints collection (artworks created in 3D software, surreal compositions). Museum-grade giclee on archival paper, framed or unframed, limited editions, and a wholesale/trade program. Price positioning: self-positioned as "affordable modern art prints", but reviewers describe prints as "on the expensive side though quality justified it" — effectively mid-to-premium for the print market.
- **Ordering Model:** Cart (standard e-commerce with cart page), plus an artwork upload & frame preview tool (frames.eastsidestudiolondon.co.uk); wholesale via trade-pricing page enquiry.
- **Strengths:**
  - Strong social proof: 155 reviews at 4.86 average on Reviews.io, plus a Trustpilot presence
  - Premium production story: 12-pigment giclee, archival paper, hand-crafted frames
  - Print-on-demand from 4 regional studios (UK/NL/AU/US) enables worldwide shipping with lower waste
  - Ethical differentiators: artists get 50%+ of profits, 1% of sales to charities (AKT, Women's Aid, CALM)
  - Interactive frame preview / artwork upload tool reduces purchase friction
- **Weaknesses:**
  - No blog or journal visible in search — zero editorial SEO despite an art-education-friendly niche
  - Mixed quality reviews exist ("poor quality print and cheaply framed... overpriced despite a discount")
  - Lookalike domain (eastsidestudiolondonstyle.shop) and ScamAdviser checks surfacing in search suggest brand-impersonation/trust noise around the brand
  - 3D/CGI is just one collection among many — not a deep specialization; product is flat prints only, no tactile/dimensional goods
- **Content & SEO strategy:** Ranks via well-structured collection and landing pages (shop by size, shop by style, all artists, best sellers, limited editions, wholesale). Clean commercial-intent SEO but no visible top-of-funnel content; discovery appears driven by collections, Instagram and Facebook.
- **What ResinRiva will do better:**
  - ResinRiva sells dimensional, tactile made-to-order resin and 3D-printed pieces with per-product customization forms — a materially richer product than flat prints of 3D renders
  - ResinRiva's 50+ post blog engine and schema-marked-up catalog capture informational search traffic East Side Studio completely ignores
  - WhatsApp-first concierge ordering plus workshops page fit India's gifting and experience market better than a pure UK cart flow

### ArtResin (reference brand — supplies, not a direct competitor)
- **URL · Country:** https://www.artresin.com · North America — site says made in the USA; sold globally via Amazon and art retailers; in India only through importers like Ubuy
- **Niche / Focus · Product Types · Price Positioning:** Epoxy resin supplies brand — "the original epoxy for resin art": crystal-clear 1:1 kits from 8oz starter to large pro sizes, UV/HALS stabilized against yellowing, ASTM D4236 certified non-toxic, food-safe claims; sells materials to resin artists, not finished art. Price positioning: premium within resin supplies — third-party review content notes users find it good but "very similar to other products on the market that are almost half the price".
- **Ordering Model:** Cart (own store with free US delivery) plus broad retail distribution (Amazon, Jerry's Artarama, etc.).
- **Strengths:**
  - Massive content moat: 750+ YouTube tutorial/instructional videos and a deep blog (beginner guides, project ideas, seasonal crafts, "what resin art sells best" business guides)
  - Safety certification story (non-toxic, no VOCs, ASTM D4236) as a clear differentiator
  - Simple 1:1 mixing ratio positions it as the beginner-friendly default, feeding a loyal fanbase
  - Full-funnel presence: blog, YouTube, Instagram, Pinterest, TikTok, newsletter — the category benchmark for content marketing
- **Weaknesses:**
  - Supplies-only: does not compete for finished-piece buyers, and its content sends demand to DIY rather than commissions
  - Price premium is contested — reviewers point to near-equivalent products at roughly half the price
  - No local India presence — available only via import resellers (Ubuy), leaving Indian resin-art demand to local brands
- **Content & SEO strategy:** The category's SEO leader: ranks for "resin art for beginners", how-to guides, video tutorials hub, project roundups, and resin-business advice. Content strategy = educate beginners into customers; blog + video library are the model ResinRiva's blog engine should localize.
- **What ResinRiva will do better:**
  - ResinRiva monetizes the finished-goods side of the same interest graph — buyers who want luxury resin pieces, not chemistry — with a structured catalog, price bands and WhatsApp customization ArtResin has no equivalent for
  - ResinRiva's workshops page can capture the Indian DIY/learning demand that ArtResin's content creates but cannot fulfill locally (no India distribution)
  - ResinRiva can win India-specific informational SEO (care guides, festive gifting, resin in Indian climate) where ArtResin's content is US-centric

### Printables (trends reference only — not a competitor)
- **URL · Country:** https://www.printables.com · Czech Republic (operated by Prusa Research); global community
- **Niche / Focus · Product Types · Price Positioning:** 3D model database and maker community: free STL models plus the Printables Store for paid user-published models (Stripe payouts, seller T&Cs), design contests and annual Printables Awards, official brand profiles from major companies; Home Decor category under Household tracks what maker-decor is trending (vases, geometric pots, print-in-place designs, shadow-effect lamps, charging stands). Price positioning: free models plus paid store models (digital files only) — not a physical decor seller, so no goods pricing.
- **Ordering Model:** Digital download platform — free downloads and cart-style purchase of paid model files; no physical fulfillment.
- **Strengths:**
  - Huge model library with the cleanest Creative Commons license tagging and highest density of CC-BY uploads (relevant for legally selling prints)
  - Contests and Printables Awards (e.g. Practical & Functional Objects category) act as a live signal of design trends
  - Official brand profiles bring industry names publishing official printable models
  - By Feb 2026 search mixes free and exclusive Store/Club models — the platform is actively commercializing
- **Weaknesses:**
  - Serves makers who own printers, not decor buyers — no fulfillment of physical goods
  - Discovery is model-centric and utilitarian, not curated or luxury-oriented
  - Commercial use of models requires license diligence (CC BY-NC pitfalls), a barrier for casual sellers
- **Content & SEO strategy:** Dominates "3D models" and category queries (Home Decor, Household, Bedroom); the Prusa blog drives platform news (Store launch, Awards, brand profiles). Useful as a trend-mining source: what tops its home-decor category tends to become the next wave of sellable printed decor.
- **What ResinRiva will do better:**
  - ResinRiva delivers finished, quality-checked printed pieces to customers with no printer — a completely different buyer than Printables' makers
  - ResinRiva can trend-mine Printables' Home Decor category and contests to seed new catalog items and blog posts before Indian competitors notice them
  - ResinRiva's structured catalog with price bands, schema markup and customization forms turns raw model trends into a shoppable luxury experience

---

## Marketplace lens

### Etsy resin & epoxy sellers (market)
- **URL · Country:** https://www.etsy.com/market/resin_wall_art · Global (US platform; localized Etsy India storefront at etsy.com/in-en with INR pricing)
- **Niche / Focus · Product Types · Price Positioning:** Aggregated marketplace channel, not a single competitor: millions of handmade/vintage sellers listing resin wall art, ocean/geode paintings, resin lamps and dioramas, epoxy river tables, coaster/tray sets, charcuterie boards, and Indian-wedding varmala/jaimala preservation frames. Heavy overlap with ResinRiva's made-to-order categories. Price positioning: very wide bands visible in search snippets — resin coasters/trays $4.40–$70; epoxy river tables $186–$7,500 on Etsy (industry sources cite up to $25,000+ depending on size/wood); Etsy India resin art listings from ~₹782, mid-range ₹5,079–₹18,997, premium pieces ₹50,074+ with outliers at ₹1,81,038. Effectively spans commodity to luxury with no positioning control for any one seller.
- **Ordering Model:** Cart checkout with structured personalization: sellers define up to 5 custom text fields per listing ("Add your personalization" box); beyond that, buyers must use Etsy Messages to negotiate a private custom-request listing. No WhatsApp; all communication is inside Etsy's messaging system.
- **Strengths:**
  - Dominant SEO: programmatic market/category pages (e.g. "Buy Resin Art Online In India – Etsy India") rank for virtually every resin long-tail keyword, including varmala preservation
  - Massive built-in buyer traffic plus trust infrastructure: reviews, buyer protection, secure payments, gifting features
  - Native personalization flow (up to 5 custom fields per listing) plus a private custom-request listing mechanism for one-of-a-kind commissions
  - Localized India storefront (etsy.com/in-en) showing INR prices, giving Indian sellers international reach
  - Bestseller/social-proof signals (badges, review counts) that help buyers pick among made-to-order sellers
- **Weaknesses:**
  - Heavy fee stack for Indian sellers: $0.20 listing fee, 6.5% transaction fee, 3% + ₹15 payment processing, 0.29% India regulatory operating fee, 2.5% currency conversion, USD-only payout via Payoneer — effective take of ~10–13% (up to ~28%+ with Offsite Ads) per multiple fee guides
  - Extreme saturation: millions of sellers, and search-result commentary notes mass-produced goods mixed with genuine handmade, with the algorithm reported to favor larger shops and ad spenders
  - Sellers own no customer relationship or brand surface — communication, branding and repeat business are locked inside Etsy; no WhatsApp or direct contact allowed
  - Personalization is capped at 5 text fields, so complex made-to-order resin specs (size, colour palette, embedded objects, photos) spill into slow back-and-forth messaging
- **Content & SEO strategy:** Etsy itself ranks via thousands of auto-generated market pages ("Resin Wall Art - Etsy", "Buy Resin Art Online In India - Etsy India"); individual sellers get no independent SEO footprint, no blog, no schema of their own — their visibility is rented from Etsy search and ads.
- **What ResinRiva will do better:**
  - ResinRiva keeps 100% margin and the full customer relationship: per-product customization forms feed directly into WhatsApp — richer than Etsy's 5-field personalization cap and with none of the ~10–28% fee stack Indian Etsy sellers face
  - ResinRiva owns its SEO surface (full schema markup, OG cards, sitemap, 50+ post blog engine, portfolio case studies) instead of renting visibility inside a saturated marketplace algorithm
  - Structured catalog with price bands plus a workshops page lets ResinRiva control luxury positioning end-to-end, whereas Etsy juxtaposes ₹782 commodity listings next to premium pieces and erodes price integrity

### Artsty
- **URL · Country:** https://www.artsty.com/ · India
- **Niche / Focus · Product Types · Price Positioning:** Marketplace-adjacent "emotional gifting" platform (Shopify store aggregating work by Indian artisans, especially women creators): customized resin gifts, varmala/wedding-garland preservation blocks and frames, resin wall clocks, corporate gifting (branded coasters, fridge magnets, resin awards), plus DIY workshop sessions hosted locally or via YouTube/Instagram Live. Price positioning: mid-market with aggressive discounting — varmala preservation from Rs 980; sale prices Rs 1,500–4,280 against struck-through "regular" prices of Rs 2,000–7,000 (e.g. Rs 3,280 vs Rs 5,200; Rs 4,280 vs Rs 7,000); promo of a free ₹1,000 gift on orders over ₹3,000.
- **Ordering Model:** Shopify cart checkout; customization handled off-flow — customers email photos/names/requirements to order@artsty.com or WhatsApp 9330723331; free pickup of flowers for preservation; stated making + delivery windows of 1–25 days each.
- **Strengths:**
  - Deep varmala-preservation catalog (hexagon blocks, square blocks with photos, LED wooden stands, framed pieces) ranking for varmala keywords alongside Etsy
  - Free flower pickup for preservation orders — a strong service differentiator for wedding keepsakes
  - Corporate gifting and DIY workshop offerings broaden revenue beyond retail gifts
  - Active Instagram presence (@artsty.in, positioned as "Emotional Gifting") plus a blog making the market-size case for resin gifting in India
  - Artisan-collective story (Indian women creators) gives an emotional brand hook
- **Weaknesses:**
  - Weak SEO hygiene: many indexed product URLs are duplicated "copy-of-..." Shopify slugs (e.g. /products/copy-of-beautiful-epoxy-resin-varmala-preservation), and the homepage title even misspells the brand ("Artsy | ... – Artsty")
  - Discount-heavy presentation (strikethrough pricing, free-gift promos) reads mid-market, not luxury
  - Customization is unstructured — buyers must email or WhatsApp requirements manually rather than through per-product forms
  - Thin content engine: only one blog post surfaced in search results; customer reviews not visible in search results
- **Content & SEO strategy:** Ranks for varmala-preservation and resin-gift queries mainly via individual Shopify product pages; blog presence is minimal (a single "Learn Resin Art & Build an Income" post surfaced); content strategy appears Instagram-first rather than SEO-first.
- **What ResinRiva will do better:**
  - ResinRiva's structured catalog with price bands and per-product customization forms feeding WhatsApp beats Artsty's "email us your requirements" flow — lower friction and fewer lost specs
  - Full schema markup, OG cards and sitemap versus Artsty's duplicated "copy-of" slugs and mistitled homepage gives ResinRiva a durable technical-SEO advantage on the same varmala/resin-gift keywords
  - ResinRiva's luxury positioning (fast UI, portfolio case studies, no strikethrough discount clutter) targets the premium band Artsty dilutes with perpetual sale pricing; its 50+ post blog engine and workshops page out-publish Artsty's near-empty blog

---

## Scrape-source candidates discovered

New scrape-source candidates surfaced during this research, **deduped by domain and excluding every domain already present in the seed registry at research time** (the registry data now lives in `src/lib/scraper/seed-data.ts` — 116 curated sources, reconciled on every deploy; `seed-sources.ts` only re-exports it). Several candidates below have **since been absorbed into the registry** — marked *(now in registry)* in their rows. Etsy/Amazon/IndiaMART/Myntra listings, Linktree pages, and pure B2B 3D-print service bureaus (iamRapid, Makenica, 3Ding, Zbotic, A3DXYZ, Soch3D, Rapid3d) were excluded per the research rules.

> Candidates are added to the Scraper registry only after a live fingerprint verification (Studio → Scraper → Sources → Add) — this environment's network policy blocked direct probes, so verification happens in production per the spec's mandatory-verification rule.

| Name | URL | Suggested tier | Country | Evidence note |
|---|---|---|---|---|
| Varmala Preservation India | https://varmalapreservation.com/ | RESIN_GOODS | India | Sister brand of Resin Arts Jaipur (Kavita Dadhich); dedicated wedding garland/flower preservation store with 54+ paginated shop pages (Woo /shop/), city landing pages, own 4.8/5 reviews page, all-India pickup-and-drop service — strongest niche-preservation SEO play seen |
| ResinVilla | https://www.resinvilla.com/ (also resinvilla.store) | RESIN_GOODS | India (Pune) | Shopify store with price-band collections (under 500/1000/5000) and city collections (Pune, Hyderabad, Delhi-NCR); varmala/flower preservation, LED lamps, jewellery, plus an art-supplies section |
| Resin Art Store India | https://resinartstore.in/ | RESIN_GOODS | India (Lucknow) | Handmade keychains, coasters, tables, trays, engagement platters + varmala preservation; email/WhatsApp support; city product-tag pages; ranks in "best resin art store" searches |
| MT Resin Arts India | https://mtresinartsindia.com/ | RESIN_GOODS | India (Jaipur) | Claims 15 years in epoxy resin; sells products AND runs an academy (offline workshops, online courses, YouTube tutorials) — hybrid goods-plus-training model — *(now in registry)* |
| Resin Art Jaipur | https://www.resinartjaipur.com/ | RESIN_GOODS | India | Separate site nearly identical in name to seeded resinartsjaipur.com — epoxy tables, resin gifts and preservation in Jaipur; evidence of name-collision competition in that market |
| Akanksha Arts Studio | https://www.akankshaartsstudio.com/ | RESIN_GOODS | India (Nagpur) | Resin art products plus workshops/classes in Pune and Nagpur — workshop-led lead generation model |
| Resineera | https://resineera.com/ | RESIN_GOODS | India (Delhi NCR) | "Premium Resin Art, Decor & Workshops"; varmala preservation ₹2,799–₹8,499 (frames with photo ₹2,999–₹7,999); city workshop SEO landing pages (Delhi, Noida, Gurgaon) plus online workshops with material kits — closest positioning match to ResinRiva — *(now in registry)* |
| Resin Artology | https://resinartology.in/ | RESIN_GOODS | India | Shopify store with a varmala preservation resin art collection |
| Shaahi Decor | https://shaahidecor.com/ | RESIN_GOODS | India | Shopify /collections/varmala-preservation plus resin home decor accessories; "directly from resin artist" positioning |
| Artsty | https://www.artsty.com/ | RESIN_GOODS | India | Shopify /products/; customized resin epoxy varmala preservation incl. LED wooden stand variants (profiled above under Marketplace lens) |
| Art and Designs | https://www.artanddesigns.in/ | RESIN_GOODS | India | Personalized resin gifts: alphabet lamps, bookmarks, car hangings, clocks, coasters, frames, rakhis, wedding trays; claims ASTM D-4236 certified safe resin |
| Resin Art by Kainaat | https://resinartbykainaat.com/ | RESIN_GOODS | India | Resin keychains and small-format resin products |
| Resinhastkala Store | https://resinhastkalastore.com/ | RESIN_GOODS | India | Surfaced for resin keychain/coaster ordering queries; details not visible in search results |
| Viva Gifts | https://vivagifts.in/ | RESIN_GOODS | India | WooCommerce /product-category/ with a varmala preservation category under resin products — from Rs 800 with doorstep pickup & delivery, aggressive entry-price signal |
| ResinClocks.in | https://resinclocks.in/ | RESIN_GOODS | India | Single-niche "luxury resin wall clock" domain — exact-match-domain SEO play, all sizes/colors — *(now in registry)* |
| Resin By Ren | https://resinbyren.com/ | RESIN_GOODS | UK | Handmade luxury resin trays and coaster sets by artist Rengin; dedicated custom-orders collection, heat-resistant serving trays — direct stylistic peer for ResinRiva |
| Coastal Coasters | https://www.coastalcoastersli.com/ | RESIN_GOODS | USA (New York) | Hand-painted resin coaster sets and serving trays positioned as "affordable luxury" handmade home gifts |
| The Resin Society | https://resinsociety.net/ | RESIN_GOODS | not visible in search results | Resin home decor and epoxy tables plus materials/tools — hybrid goods+supplies model — *(now in registry)* |
| Merchlist | https://themerchlist.com/ | RESIN_GOODS | not visible in search results | Custom resin coasters with name/logo for corporate gifting and promo — validates a B2B corporate-gifting angle for resin goods |
| Pourfect | https://pourfect.in/ | SUPPLIES | India | ASTM-certified, non-toxic, food-safe made-in-India resin kits; claims 10,000+ artist community; serves the DIY segment ResinRiva's workshops touch |
| Epoxy Art India | https://epoxyartindia.com/ | SUPPLIES | India | WooCommerce /product-category/; premium epoxy resin, pigment sets, glitters, dried flowers for resin at Rs 80–220/pack — *(now in registry)* |
| ResinCult | https://resincult.com/ | SUPPLIES | India | Resin brand using "Best Resin Brand in India (2025)" self-ranking blog SEO |
| MB Epoxy | https://mbepoxy.in/ | SUPPLIES | India | Indian epoxy/art-resin supplier ranking for "art resin" queries in India — local supplies competitor context — *(now in registry)* |
| Itsy Bitsy | https://itsybitsy.in/collections/resin-art | SUPPLIES | India | Large craft retailer with a resin art materials collection |
| Artsshala | https://artsshala.com/ | SUPPLIES | India | Craft store publishing SEO blog content like "The Guide to Resin Art Trends in India" — content-strategy competitor (inferred supplies tier) |
| BohriAli | https://bohriali.com/ | SUPPLIES | India (Pune) | Official Jesmonite AC100 India distributor; 400+ moulds; runs artist-led workshops AND a blog ("Jesmonite in India: Everything You Need to Know") — rare content player — *(now in registry)* |
| JorShor | https://jorshorstore.com/ | SUPPLIES | India (Ahmedabad) | Shopify /collections/ for resin pigments (mica, pearl, opaque, metallic) and silicone moulds; physical store — *(now in registry)* |
| The Mould Story | https://www.themouldstory.com/ | SUPPLIES | India (Mumbai) | Made-in-India silicone moulds for resin, baking, soaps, chocolates |
| Brownwoods4art | https://brownwoods4art.com/ | SUPPLIES | India | Shopify /collections/flowers; real + artificial dried/preserved botanicals for resin jewellery, coasters, frames — *(now in registry)* |
| Resin Obsession | https://resinobsession.com/ | SUPPLIES | USA | Content-led supplies brand ("artist's #1 choice", expert support); separate shop subdomain — benchmark for education-driven SEO |
| Time To 3D | https://timeto3d.com/ | PRINT3D | India | Claims to be India's first/highest-rated 3D printing hub; Shopify /collections/customised-gifting — dedicated personalised gifting line |
| The 3D Printing Store India | https://the3dprintingstore.in/ | PRINT3D | India (Bangalore) | Shopify; 801 judge.me reviews at 5.0, 90k+ customers claim, eco bio-plastic positioning, Rs 400–2,000 gifts (profiled above) |
| 3DPrintShop | https://3dprintshop.in/ | PRINT3D | India | Custom 3D prints from decor and gifts to functional parts; name/logo/message personalization |
| Apsara Gifts | https://www.apsaragifts.one/ | PRINT3D | India (Bangalore) | Affordable 3D printed products, no minimum order quantity, ships India + international |
| Drag And Drop | https://draganddrop.design/ | PRINT3D | not visible in search results (EU-style design studio, en-us storefront) | Nature-inspired large-scale 3D printed "ceramic" vases and lampshades using bio-based and recycled materials; sustainable design positioning |
| Sonogo | https://www.sonogodesign.com/ | PRINT3D | not visible in search results | Designer Jovi Lim's curated collection of plant-based 3D-printed lamps; boutique sustainable-design angle |
| Terra Labs | https://terralabs.design/ | PRINT3D | not visible in search results | Sustainable 3D-printed lamps and furniture ("3D Printing Lamps of the Future"); table lamps blending form and function |
| SLIMPRINT | https://slimprintshop.com/ | PRINT3D | not visible in search results (EU inferred) | Studio turning bio-plastic waste into design pieces; every vase made to order and hand-packaged — closest to ResinRiva's made-to-order model |
| JONATHAN Y | https://www.jonathany.com/collections/3d-printed-lamps | PRINT3D | USA | Mainstream home-decor brand with a dedicated 3D printed lamps collection — signals the category going mass-market |

---

## Summary

### Market gaps

- **No competitor combines luxury positioning with transparent price bands.** The Indian market splits into discount-strikethrough carts (SAASHI, Woodensure, Artsty, ResinVilla) and quote-only premium studios (Resin Arts Jaipur, Poonam Shah Art) — ResinRiva's structured price-band + WhatsApp concierge model is uncontested.
- **Nobody spans resin art AND 3D printing under one premium brand.** Only Leoberry Gifts does both, and at budget positioning; across the full discovery sweep no other brand combines the two — ResinRiva's dual-category luxury position is nearly unique.
- **Structured customization is absent everywhere.** WhatsApp appears widely as a support/custom-quote channel (Pacific, Artsty, Woodensure, T3DPS), but no competitor surfaced has per-product customization forms feeding a pre-structured WhatsApp order — ResinRiva's form-to-WhatsApp flow is a genuine differentiator.
- **The luxury-resin search demand is up for grabs.** The one true luxury player (Poonam Shah Art) has ~5 indexed pages, no catalog, no pricing, no blog; "luxury" is claimed in title tags (ResinClocks.in, Pacific) but the stores are template Shopify catalogs — the true luxury-UI + SEO position is open.
- **Finished-goods brands barely blog.** Almost no finished-resin-goods competitor shows editorial content; the only visible content players are supplies stores (BohriAli, Artsshala, ResinCult) and global supplies brand ArtResin (750+ YouTube videos, deep blog). Informational resin queries in India are nearly uncontested by product brands.
- **Varmala/flower preservation is the hottest — and least price-transparent — commercial cluster.** 6+ dedicated sites including exact-match domains (varmalapreservation.com, varmalapreservationindia.com), LED-stand variants, and pickup-and-drop logistics as the differentiator; premium studios quote-gate pricing, so high-intent "varmala preservation cost" queries are underserved. Worth a dedicated ResinRiva landing page + case study, not just a blog post.
- **Technical SEO hygiene is weak across the field:** duplicated "copy-of" Shopify slugs and a mistitled homepage (Artsty), split domains (Pacific .in/.com, Woodensure's Odoo blog subdomain), indexed 404s (Woodensure), bot-verification pages in the index (Leoberry), thin crawl footprints (Kanha Kreation, Resinart.in) — schema/OG/sitemap discipline is a durable edge.
- **Etsy's India fee stack (~10–28% effective take, USD payout via Payoneer) makes direct-to-WhatsApp selling economically attractive** — a strong "why buy/sell direct" narrative for both content and positioning.
- **Proven playbooks worth adopting:** city landing pages rank (Sumaiya's "resin art in [city]", ResinVilla's city collections, Resineera's Delhi/Noida/Gurgaon workshop pages); Web Stories are used only by Sumaiya; workshops are a validated revenue + lead-gen channel (Resin Arts Jaipur, MT Resin Arts, Resineera, Artsty, BohriAli) — supports giving ResinRiva's workshops page city-keyword treatment.
- **India's resin e-commerce skews to supplies/DIY, not finished luxury goods**; the professionalization of 3D-printed decor visible in EU/US (Decords, SLIMPRINT, Terra Labs, JONATHAN Y) has no visible Indian equivalent — an early-mover window for ResinRiva's 3D-printed decor line.
- **Buyers accept long made-to-order lead times when the process is explained** (SEAWAVETABLE 45–55 working days, Woodensure 30 working days) — supports ResinRiva's made-to-order WhatsApp + customization-form flow with clearly stated timelines.

### Pricing insights for India

All figures from search-result snippets (July 2026); verify live before setting ResinRiva price bands.

- **Keychains:** Rs 40–250 (Sumaiya Rs 40–150 budget floor; SAASHI Rs 245–249 discounted from Rs 299–399)
- **Resin jewellery:** Rs 199–399 (SAASHI)
- **Coasters:** branded singles Rs 270–420 (Leoberry); commodity/IndiaMART as low as Rs 40–49/pc; coaster sets Rs 799–999 (SAASHI, struck through from Rs 1,800)
- **Photo frames:** from Rs 420 basic (Leoberry) and from Rs 799 (Sumaiya) up to Rs 3,199–3,899 (Leoberry wooden resin frame)
- **Wall clocks:** Rs 1,999+ (Pacific) to Rs 2,299–2,899 (SAASHI)
- **Pooja thalis:** Rs 2,000–2,200 (Kanha Kreation, 10-inch)
- **Trays / tray sets:** Rs 3,500–3,999 (Kanha Kreation)
- **Geode/ocean wall art:** Rs 4,000–6,000 (SAASHI)
- **Small resin tables:** Rs 3,000–7,500 (SAASHI); **epoxy river dining tables:** Rs 25,000–1,00,000+ (Woodensure example Rs 55,800)
- **Varmala preservation:** the widest, least-anchored band — entry Rs 800 (Viva Gifts, with doorstep pickup) and from Rs 980 (Artsty); common service floor Rs 2,999–4,999; Pacific from Rs 2,499; Resineera ₹2,799–₹8,499; Artsty sale Rs 1,500–4,280 vs "regular" Rs 2,000–7,000; premium studios (Resin Arts Jaipur) fully quote-gated
- **Wedding return gifts:** pitched at Rs 500–2,000 (Leoberry blog)
- **Anniversary gifts:** from Rs 399 (Sumaiya)
- **3D printed personalized gifts:** Rs 400–2,000 (The 3D Printing Store India), with a Rs 100 priority-dispatch (3-day) upsell over free 10-day dispatch
- **Etsy India resin art:** ~₹782 entry, ₹5,079–₹18,997 mid, ₹50,074+ premium (outlier ₹1,81,038); Etsy US coasters $4.40–$70, epoxy river tables $186–$7,500 (industry guides: $5,000–6,000 for a standard 8ft × 42in table, up to $25,000+)
- **Marketplace economics:** Etsy's effective take for Indian sellers ~10–13% (up to ~28%+ with Offsite Ads) — direct WhatsApp selling keeps that margin
- **Margin signal:** Artsty's own blog claims ₹150–300 cost-to-make items retail at ₹800–2,000+ in gifting
- **Adjacent reference points:** dried flowers for resin Rs 80–220/pack (Epoxy Art India); supplies free-shipping threshold Rs 5,000 (Tulsi); a resin workshop listed at ₹8,100 (Hobby Workspace); 3D print service floors ~Rs 800 minimum order / ~Rs 3 per cm³ (B2B bureaus)
- **Lead-time norms to communicate:** small resin gifts 3–7 days curing/production; 3D printed gifts 3–10 days dispatch; resin furniture ~30 working days (Woodensure) to 45–55 working days (SEAWAVETABLE)

### Product & blog opportunity list

Opportunities ResinRiva's Phase 11 blog engine, category structure, workshops page, and portfolio case studies already target — plus new ideas surfaced by this research.

**Product / page opportunities:**

- **Varmala preservation as a flagship category:** dedicated landing page + portfolio case study, transparent price bands (the Rs 800 → ₹8,499 market band is unanchored), LED-stand and framed variants, and a pickup-and-drop logistics story (the differentiator used by varmalapreservation.com, Viva Gifts, Artsty)
- **Devotional line:** resin pooja thalis and mantra/Navkar frames (Kanha Kreation and Resin Arts Jaipur validate demand; nobody owns it nationally)
- **3D-printed decor line for India:** vases, shadow/lithophane lamps, geometric planters — trend-mine Printables' Home Decor category and the EU/US wave (Decords, SLIMPRINT, JONATHAN Y) with no visible Indian equivalent
- **Lithophane photo gifts:** Amazon/Etsy dominate the query with no premium brand — brandable gap
- **Corporate/B2B gifting:** logo coasters, resin awards, bulk return-gift keychains and employee kits (validated by Leoberry, Artsty, Merchlist, Sumaiya's bulk pages)
- **Big-ticket tier:** geode/ocean wall art and resin tables with clearly explained made-to-order lead times (SEAWAVETABLE proves buyers accept 45–55 days when the process is explained)
- **Price-band browse pages** (under ₹500 / ₹1,000 / ₹5,000 — ResinVilla's pattern) and **city landing pages** ("resin art in Chennai/Mumbai/Pune/Delhi/Lucknow" — Sumaiya/ResinVilla precedent)
- **Workshops with city-keyword treatment** (Resineera's Delhi/Noida/Gurgaon pages rank; LBB workshop listicles rank) — plus workshop content that converts the supplies-store hobbyist audience (Tulsi's claimed 1-lakh community)

**Blog topic opportunities (grounded in observed rankings and gaps):**

- "Varmala preservation cost guide India — what ₹800 vs ₹2,500+ vs ₹8,500 actually gets you" (competitors quote-gate this)
- "How varmala pickup & drop works" (logistics differentiator explainer)
- "Resin art price guide by product type" (only Sumaiya publishes prices, in blog prose)
- "Epoxy resin dining table cost in India" buyer's guide (Woodensure and US sites rank)
- "Resin wall clock buying guide (sizes, colors, pricing)"
- "Resin frame size guide (4–10 inch)" (mirrors Leoberry's structure)
- "Wedding return gifts by budget" and city gift guides ("resin gifts in [city]")
- Anniversary/wedding occasion posts with Hindi-relational keywords (e.g. bhaiya-bhabhi — Sumaiya's pattern)
- Dated trend listicles ("Top resin gift ideas 2026") and "Best resin art websites in India" style listicles (Kanha Kreation, ResinCult use self-ranking listicles)
- "Resin vs 3D printed home decor" and "3D printed vs resin nameplates" (hybrid queries only ResinRiva can own)
- "Resin vs Jesmonite: which to choose" (contest BohriAli's ranking article; Poonam Shah uses Jesmonite, nobody explains it)
- "Non-yellowing resin: why it matters for keepsakes" (Viva Gifts leads with this claim) and resin care/heat-resistance/durability explainers for serveware
- "Resin art for beginners" India-localized (ArtResin proves education converts; its content is US-centric with no India distribution)
- "How to brief a custom resin commission (photos, sizes, colours)" and "How custom made-to-order works (lead times, sketch to piece)"
- "Etsy fees for Indian resin sellers vs selling direct" (the ~10–28% fee-stack story)
- "Lithophane photo lamp gift guide" and 3D-printed vase / shadow-lamp trend posts (seeded from Printables/MakerWorld category trends)
- "Resin coaster corporate gifting in India" (B2B keyword cluster)
- "Choosing a resin art workshop in Delhi/NCR" and other city workshop guides
- Renter-friendly decor without nails (Decords' hook, unclaimed in India)
- Pooja thali and mantra frame occasion/devotional content
- Google Web Stories for gift listicles (only Sumaiya uses the format)

---

*Research completed July 2026. All competitor claims are search-snippet-based (direct page fetches were blocked in the research environment); re-verify any load-bearing figure live before pricing or copy decisions. Competitor material is reference-only — never copy text, product names, descriptions, or images.*
