...
| `docs/`      | The documentation tree. **Start at `docs/README.md`** — half of what is there is dated or superseded, and that page is what tells them apart. Three paths in it are pipeline inputs that must not move. `docs/reference-design/` is the reference design (`implementation-plan.md` §4.5 and §6, `awwwards-redesign-spec.md`, the UI/UX audit, `mockup/`) — reference, **not law**; `REDESIGN.md` outranks it |
