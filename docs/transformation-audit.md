...
**Verdict: the master prompt describes a site that is roughly three-quarters built. Of its 89 sections, 18 are already satisfied, 54 are partly built, 2 are blocked by data the business does not have, 8 need an owner decision before any code is written, 3 are missing outright (the design lab as specified, and the two testimonial demo-data sections that wait on the testimonial schema), and 4 are process. Eighteen individual asks inside those sections are forbidden by the binding design spec and hard rules the prompt itself says to honour (§4.3), and twenty inspiration-library pattern families were screened out for the same reasons (§14).**
...
Plus the five Phase 2e questions that remain open and unchanged (`PROJECT_STATE.md`, "Phase 2e — OPEN QUESTIONS"): `/shop?q=` searching descriptions, the "Show all N pieces" label, group-aware breadcrumbs, canonicals for `?type=`, pre-2a bookmarks; and the Phase 2b question of swapping the hero's primary and secondary CTAs under commission-led positioning.
...
**Counts:** EXISTS 18 · PARTIAL 54 · DATA_GAP 2 · DECISION 8 · MISSING 3 · PROCESS 4. Forbidden asks: 18 (§4.3); forbidden library pattern families: 20 (§14).
