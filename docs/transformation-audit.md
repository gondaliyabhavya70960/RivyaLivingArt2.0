...
| D18 ✅ **DONE (2026-09-03)** | Dormant v2 files | — | Nine deleted: Preloader, preloader-signal, CursorFollower, Magnetic, HeroParallax, KineticHeading, SplitTextHeading, mobile-whatsapp-bar, wishlist-count | Three of the listed files were NOT dormant and stay: `product-card` and `order-summary-preview` are imported by `/design-lab` (whose own fate is D17), and `tabs` has 19 live references. Symbol-level search found zero references to the nine; typecheck, lint and 383 tests confirm it. |
...
| D23 ✅ **DONE (2026-09-03)** | Legacy un-gated sheet push | — | `syncSourceToSheet` bypassed `SheetSyncPolicy` | Removed. MANUAL sources now stop appearing in the tier tabs until "Add to Sheet" is pressed — which is what the policy always claimed. `sheet-policy.test.ts` asserts the one-writer invariant on the source. |
| D24 ✅ **DONE (2026-09-03)** | Housekeeping requiring owner approval | — | Four workflows deleted, not six: `mirror-images`, `mirror-v3-media`, `mirror-v6-media` and `fetch-assets` each state in their own header that they are safe to delete once the assets are committed, and `public/` was committed on 2026-08-31. `fetch-media-v3`, `fetch-media-v3-video` and `fetch-tiers` STAY — CLAUDE.md, `data/tiers/README.md`, `docs/google-sheets.md` and `scripts/media-v3-video-fetch.mjs:89` all name them as the documented way to rebuild the masters or refresh the tier snapshot, and no sandbox can reach those hosts. Also deleted: `.playwright-mcp/` (6 files) and the two regenerable `audit/*.json`, now gitignored. | Superseded docs are **bannered in place, not moved**: `DESIGN.md` alone has 164 inbound references, many in dated records where rewriting the path would falsify them. `.env.example` remains the owner's. |
