...
**Verdict: the master prompt describes a site that is roughly three-quarters built. Of its 89 sections, 18 are already satisfied, 53 are partly built, 4 are blocked by data the business does not have, 9 need an owner decision before any code is written, 1 is genuinely missing, and 4 are process. Eighteen individual asks inside those sections are forbidden by the binding design spec and hard rules the prompt itself says to honour (§4.3), and twenty inspiration-library pattern families were screened out for the same reasons (§14).**
...
| Routes | 21 public under `src/app/[locale]/(v2)` · 46 Studio pages under `src/app/studio` · `/design-lab` (dev-only, 404 in production) · 9 API routes + `/uploads/[...path]` |
...
**Counts:** EXISTS 18 · PARTIAL 53 · DATA_GAP 4 · DECISION 9 · MISSING 1 · PROCESS 4. Forbidden asks: 18 (§4.3); forbidden library pattern families: 20 (§14).
