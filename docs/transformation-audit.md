...
**Verdict: the master prompt describes a site that is roughly three-quarters built. Of its 89 sections, 19 are already satisfied, 52 are partly built, 4 are blocked by data the business does not have, 9 need an owner decision before any code is written, 1 is genuinely missing, and 4 are process. Eighteen individual asks inside those sections are forbidden by the binding design spec and hard rules the prompt itself says to honour (§4.3), and twenty inspiration-library pattern families were screened out for the same reasons (§14).**
...
3. **There is no safe place for synthetic content.** No `isDemo` marker exists anywhere (`grep -rni isDemo prisma src scripts` returns nothing). The only demo convention is a `title startsWith "DEMO"` exclusion applied at nine Product read sites (`src/lib/shop.ts:104,675`, `src/lib/search-query.ts:47`, `src/app/sitemap.ts:34`, three route files) whose seed was deliberately deleted (`prisma/seed.ts:16-17`: "the catalog ships 100% empty and owner-fed"). `npm run build` runs `prisma/bootstrap.ts` against the production database on every deploy, and it already republishes 20 Portfolio rows each time (`bootstrap.ts:305`, `seed-portfolio-cases.ts:488`). Any demo seed wired near that path lands in production. The 100 demo products, 30 demo articles and 30–50 demo testimonials of §40–42 and Phase 15 are **FORBIDDEN in the production database** under HARD RULE 3 and are recorded here as an owner decision with a recommended isolation design (§14 below).

Everything else in the prompt is smaller than it looks. The v3 "Liquid Luxury" design system, the global chrome, all 22 public routes, the Part 9 product page, the shop and search, the eight-surface CMS, the scraper pipeline, the Sheets import/export, the media library and the nine-locale i18n layer are built, gated and live at https://www.rivyalivingart.com. What remains is refinement, a handful of buildable new bands, a set of schema decisions, and a body of hygiene that previous audits already logged.
...
| Source | 91,871 lines of `.ts/.tsx/.mjs`: `src/app` 20,829 · `src/components` 40,353 · `src/lib` 19,974 · `src/actions` 10,053 (31 modules, 124 exported Server Actions) · `src/hooks` 319 · `src/i18n` 258 · `prisma/` 2,735 · `scripts/` 4,026 |
...
| Routes | 22 public under `src/app/[locale]/(v2)` · 46 Studio pages under `src/app/studio` · `/design-lab` (dev-only, 404 in production) · 9 API routes + `/uploads/[...path]` |
| Database | 35 models, 14 enums (`prisma/schema.prisma`, 998 lines) · **44 migrations**, DDL-additive (0 `DROP TABLE` / `DROP COLUMN` / `ALTER COLUMN` / `RENAME`) — `PROJECT_STATE.md:572` still says 43 |
...
| `/large-resin-art` | `large-resin-art/page.tsx` (495 lines) | Six sections: scale hero · scope (4 tiles) · how (dark) · brief · conditional gallery (`LARGE_FORMAT_CATEGORY_SLUGS`) · commission CTA; Service JSON-LD; `LargeFormat` namespace ×9 | §21: 6 of 18 sections exist, 4 more partially; 21.3–21.8 and 21.11 DATA_GAP; 21.15–21.17 MISSING (S each, patterns exist on `/custom-order`) | REFINE |
...
| `/search` | | Whole-catalogue search with synonyms; hands off to `/shop?type=all` (Phase 2d) | §68 products only | KEEP (stale v2 comments) |
...
| `/custom-order` | Part 10 | Split hero, four commission kinds, how, brief form → Server Action → `Inquiry` → `wa.me`, FAQ, past work, testimonials | §15 EXISTS; wording differs from the prompt (owner slot) | KEEP |
...
| `/portfolio`, `/portfolio/[slug]` | §11.1–11.2 | Masonry wall, category chips, mono numbers, pagination; case page with spec rail, stats, brief, captioned gallery, `BeforeAfter` (never renders: no row has `beforeImageUrl`), VisualArtwork JSON-LD | §16 EXISTS; installation context has no field | KEEP |
...
| `product/model-viewer.tsx` | Untranslated "Loading 3D view…" (:61), gradient fill, auto-rotate not gated on reduced motion (:73), no error/retry state | i18n key, `usePrefersReducedMotion` gate, `ErrorState` on catch, poster from `images[0]` |
...
- **Quick view.** No component; four orphaned `Shop.quickView*` copy keys remain. Buildable as a storefront `Dialog` reusing `ShopProductItem` with a WhatsApp CTA, but REDESIGN §4.6 says "the whole card is the target — no large button inside the card", so the trigger must be a ghost line outside the stretched `MorphLink`.
...
| Card tilt / perspective effects / 3D cards | Product hover is "image scale 1.02–1.04 or second-image wipe"; §0 decision 2 rejects the glass/3D-panel template look | Part 14; `REDESIGN.md:24` |
...
| Per-block theme / spacing / alignment switches in the page builder (§32) | Grounds are computed by `resolveBlockGrounds` so the §3.1 violation is inexpressible; "tokens are not the editor's to change" | `docs/studio-cms/04-structure-layer.md`, `06-governance.md` |
| Stats block (§32) | The v3 redesign deleted "the undefendable stat row"; numbers only where real | `page.tsx` header, `docs/studio-cms/09` claim 9 |
| Theme settings in Site Settings (§31) | Same | `docs/studio-cms/06-governance.md` |
| Light/dark theme preview (§65) | The storefront has no theme toggle | §3.1 |
...
Seven `backdrop-blur` sites exist on the storefront against "exactly one": `site-header.tsx:383` (sanctioned), `search-overlay.tsx:441` (arguably the header's), `dialog.tsx:45`, `wishlist-button.tsx:41`, `consent-gate.tsx:74`, `use-motion-paused.tsx:122`, `mobile-whatsapp-bar.tsx:43` (unmounted). `share-buttons.tsx` pulls a Studio shadow onto the storefront. `scripts/redesign-audit.mjs` has no rule for `backdrop-filter` outside the header, `box-shadow` outside the two exceptions, hover transforms on buttons, or non-token durations; the champagne count is report-only. Adding computed-style rules is S and would make §3.5 literally true.

---

## 5. Studio

46 pages under `src/app/studio` (44 in the `(dashboard)` group plus login, signup, forgot, reset and the printable inquiry card), one shell (obsidian sidebar with champagne active rule, breadcrumb top bar, cmdk ⌘K palette, Radix Sheet mobile nav), 91 component files reusing a small primitive set (`PageHeader`/`EmptyState`, `StudioTableHead`/`StudioRow`, `BulkBar`, `usePagination`, `useSelection`, `ConfirmDeleteDialog`, `RichTextEditor`, `TranslationsSection`). Every mutation goes through the 31 action modules; 25 of them write `ActivityLog`. The Studio is English-only by design and consumes the shadcn semantic layer under the `.studio-v2` scope with its own `prefers-color-scheme: dark` block. `scripts/studio-audit.mjs` sweeps 30 authenticated routes at 1440 and 390 in CI.
...
| Top bar · breadcrumbs · page title | EXISTS | `topbar.tsx`, `breadcrumbs.tsx`, `page-header.tsx` on 45 pages |
| Tabs | PARTIAL | No `ui/tabs.tsx`; ad-hoc link groups (`blog/page.tsx:37-48`, `product-list.tsx STATUS_TABS`) and one `role=tablist` (`translations-section.tsx:78`); no tabbed product editor (§12.5) |
...
| Process · Materials | DATA_GAP / DECISION | No `Process` or `Material` model (`grep '^model'`). Both are fixed-arity copy + image slots (`Process.timeline` 6, `Home.how` 4, `about.material1-4`, `process.material1-4`) editable in site-copy/site-images. Fixed arity is documented design: "repeaters are the fastest route to a broken luxury layout" (`docs/studio-cms/04 §4.7`). Row CRUD is a new registry-pattern surface under D2 (L) |
...
- **Forms.** Labels everywhere; zod on all 7 RHF forms and re-validated in every action (`helpers.ts runAction`); `FieldError` + `aria-describedby` in blog/portfolio/page/custom-page/settings, **absent in `product-form` and `seo-form`**; helper text thin on product and SEO; success = toast only; loading via `saving` flags and `SubmitButton`; **autosave: none, and unsafe** — products, blog and portfolio save straight to the live row (no per-row draft column), so autosave would publish half-typed edits into 4,385 live products; `useUnsavedChangesGuard` is `beforeunload`-only on 7 forms (sidebar navigation loses edits silently) and absent from dialog-based CRUD.
- **Empty states.** 30 `EmptyState` call sites across products, categories, media, inquiries, blog, portfolio, testimonials, faqs, pages, users, subscribers, activity and the five scraper screens; only 5 pass an action CTA; landing pages and sheet-import use inline text.
...
| `products/product-list.tsx` (699), `product-form.tsx` | REFINE | No sortable columns; one column, no tabs, no `FieldError`, one Save (REDESIGN §12.5 asks two columns with preview) |
...
`prisma/seed.ts` writes admin user, the 16-category tree (empty of products), FAQ topics, settings and legal stubs, once. It records (:16-17) that the temporary DEMO products were removed in "Phase 10". The `title.startsWith("DEMO")` exclusion survives at nine read sites and two tests (`src/lib/shop.test.ts:9`, `tests/db/product-where.test.ts:35`) and is applied inconsistently: `sitemap.ts` applies it to blog and portfolio, the blog and portfolio list pages do not. That inconsistency is the exact failure mode an `isDemo` column would have if it were the sole safety filter.
...
4. **The Bulk Import wizard's product path upserts by slug without `decideMerge`** (`actions/import.ts:546-577`) — a third catalogue writer outside the owner-edit rule; only `needsRewrite` is preserved.
...
| Validation | EXISTS (per-row messages) | PARTIAL (rows lacking title/externalId/sourceKey dropped silently, `import-tiers.ts:271-274`) |
...
| Status | EXISTS (`sheetSyncStatus`, `sheetSyncError`, `sheetSynced`, `abortedReason`), with the caveat that tier pushes set only the job flag |
...
No 9:16 mobile crop exists for any of the twelve wide slots (`SITE_IMAGE_DEFAULT_REFS` sets `mobileUrl: null`), so every hero is centre-cropped on phones from a fresh database. `home.maker` and `about.maker` are backed by `hands-polish.webp`, which `site-images-import.test.ts:45-53` records is itself a generation — REDESIGN §15.2 says the maker is never AI; only the owner can close this.

### 10.3 Generation plan (nothing generated)

Forty items in six sets, every prompt taking the manifest's `promptSuffix` (palette `#08283A/#080A0E/#164E6B/#B89B63`, single soft key, matte, no text/logos/faces), one shared prefix per set so each reads as one session, each job recorded as a manifest entry, masters landing in `public/media/v3` as AVIF with LQIP, provenance AI, **never attached to a Product, Portfolio or Testimonial row**. Order of value: D → A1/A2/B7 → E22 → F34 → B8/B9 → C and F35–38 only after owner decisions.
...
| B. Product-linked | DATA_GAP | no `productId`; the PDP shows the global first N (`product/[slug]/page.tsx:992-1016`) |
| C. Video | DATA_GAP | no media column; component would follow `HeroMedia` (poster, touch and reduced-motion gates) |
...
| D18 | Dormant v2 files | — | Preloader, CursorFollower, Magnetic, HeroParallax (refit), KineticHeading/SplitTextHeading, product-card v2, tabs, mobile-whatsapp-bar, order-summary-preview, wishlist-count | Delete in one chore PR, or header-comment each as "unmounted on purpose — see CLAUDE.md" |
...
| D22 | Portfolio seed provenance | — | `seed-portfolio-cases.ts` republishes 20 PUBLISHED rows on every deploy | Owner confirms the 2026-08-13 brief; gate the seed on an empty table like the others |
| D23 | Legacy un-gated sheet push | — | `syncSourceToSheet` bypasses `SheetSyncPolicy` | Remove it; after removal, MANUAL sources stop appearing in the tier tabs until "Add to Sheet" is pressed |
| D24 | Housekeeping requiring owner approval | — | six mirror/fetch workflows, `.playwright-mcp/` tracked files, `audit/crawl-report.json`, `.env.example` (retired domain at :6 and :19, `ResinRiva Studio` at :12, missing `DATABASE_URL_UNPOOLED`/`RESEND_FROM`/`CRON_SECRET`; `.env*` edits are denied in-session), archived docs under `docs/archive/` | One chore PR; `.env.example` by the owner |
...
| D27 | CI | §79/§88 assume gates run | `AGENTS.md:188-190` and `PROJECT_STATE.md` KNOWN ISSUES say Actions never allocated a runner; run #71 on PR #29 proves it does (§0) | Owner retires D4's "cannot run" premise and makes CI the gate of record; local runs remain mandatory for detail routes, E2E and extra widths; correct `AGENTS.md` and `PROJECT_STATE.md` |

Plus the five Phase 2e questions that remain open and unchanged (`PROJECT_STATE.md:312-329`): `/shop?q=` searching descriptions, the "Show all N pieces" label, group-aware breadcrumbs, canonicals for `?type=`, pre-2a bookmarks; and the Phase 2b question of swapping the hero's primary and secondary CTAs under commission-led positioning.
...
| medium | Treating a green CI as sufficient: CI deliberately skips the detail routes, the E2E smoke and every width but 1440/390 | Keep D4's evidence habit for those; paste local results for what CI cannot reach |
...
- **Owner decisions** D7–D27 (above) and Phase 2e's five questions; the roadmap marks which phases each gates.
...
- **Owner actions outside the repo**: retire D4's premise now that Actions executes (D27); `.env.example`; Higgsfield credits and an ordinary-internet machine to run `scripts/media-v3-fetch.mjs` (the CDN answers 403 in-session); `GOOGLE_SERVICE_ACCOUNT_JSON` + `SCRAPE_SHEET_ID` in the environment for any Sheets work; `BLOB_READ_WRITE_TOKEN` for bundled imports; a Neon branch for the Content Lab.
...
|---|---|
| `PROJECT_STATE.md` | No §81 checkpoint block; NEXT EXACT TASK (:241-247) is marked DONE at :221 and :373-378; ":33 Ten PRs merged" vs 17 merge commits; "43 migrations" (:572) vs 44; "No new migration written yet" (:575); "CI is now live on main" (:513) vs "has never executed" (:418); test counts 29/322 vs 36/375 |
...
| Stale code comments citing `DESIGN.md B2/B4`, "Midnight Gild", "Fraunces" | `search/page.tsx`, two `loading.tsx`, `hero-media.tsx`, `template.tsx`, `dialog.tsx`, `rating-stars.tsx`, `fonts-scripts.ts:12-19` |
...
| 4 | Design direction | EXISTS | REDESIGN Part 2 / 3.5 encode the direction and the avoid-list; tokens implement it | Dormant v2 files (D18) | 1 | S |
...
| 7 | Global header | EXISTS | 8 of 9 requirements met (§2 table) | Adaptive contrast is route-declared (M); language selector placement (D12); RTL drawer edge (S) | 2 | M |
...
| 15 | Bespoke commission | EXISTS | `/custom-order` (Part 10), homepage bespoke band, footer CTA, large-format brief; no payment | Wording is an owner slot; capped parallax on the band (shortlist 9) | 3 | S |
| 16 | Portfolio | EXISTS | Masonry archive, filters, spec rail, captioned gallery, `BeforeAfter` (dormant on data), JSON-LD | Installation context has no field (D7); `beforeImageUrl` data is the owner's | 8 | S |
...
| 20 | Product detail page | PARTIAL | Part 9 PDP: gallery, identity, spec sheet, customisation, care, related, CTA, 3D chip | Visual dimensions / scale / room context DATA_GAP (D7 for numeric dimensions and `ProductImage.role`); `model-viewer` REFINE; lightbox FLIP | 6 | M |
...
| 24 | Journal / blog | PARTIAL | Featured story, grid, filters, reading progress, TOC, related posts, author-linked products | Related collections needs `BlogPost↔Category` (D7); related materials DATA_GAP; six categories vs ten is a content task | 8 | M |
...
| 27 | All website pages | EXISTS | 22 routes on v3, audited at 1440/390 + RTL | Residual REFINE: OG cards, stale comments, `/search` file docs | 2, 16 | S |
...
| 31 | Studio content management | PARTIAL | Homepage, pages, products, testimonials, journal, portfolio, FAQ, settings all editable | Collections SEO/visibility (D7); Process/Materials (D16); theme settings FORBIDDEN | 11 | L |
| 32 | Page builder | PARTIAL | Six blocks, guardrails, scheduling, translations | Grow by decision (D15): four S blocks first; video/gallery blocks after the picker; per-block theme/spacing FORBIDDEN; Stats FORBIDDEN | 11 | M |
| 33 | Design lab | MISSING | A v2 kitchen sink, 404 in production, 0 of 7 playgrounds | D17: rebuild small or retire | 10 | M |
...
| 42 | Testimonial demo structure | DATA_GAP | Needs §17 columns | After Phase 9, under D8 | 15 | S |
| 43 | Empty states | PARTIAL | `EmptyState` at 30 Studio sites and every storefront list | CTA on all Studio empties (S); landing pages and sheet-import use inline text | 10 | S |
...
| 47 | Motion performance | EXISTS | Global reduced-motion collapse, SSR-true hook, lazy GSAP, canvas confined | Automated motion-bundle check (S) | 16 | S |
| 48 | Typography | EXISTS | Display / heading / body / metadata scale, all clamped, mono numerics | — | — | n/a |
| 49 | Spacing | EXISTS | Section tiers + retained step scale for the Studio | Studio density is convention, not tokens (fine) | — | n/a |
...
| 54 | Internationalization | EXISTS | 9 × 1,181, RTL, registry, fallback | JSON-LD breadcrumb names + one loading string hardcoded (S); **stale-translation gate (S, prerequisite)** | 1, 16 | S |
...
| 57 | Image strategy | PARTIAL | Slots declare ratio, focal point, mobile crop; product images never distorted | No bundled 9:16 crops (SET E + registry field); product/portfolio images have no focal point (D7) | 12 | M |
...
| 62 | Component quality standard | PARTIAL | Six states on v3 primitives; empty/error/loading on lists | `ModelViewer` error/reduced-motion; `WishlistPanel` empty; no component-test runner | 1, 6 | S |
...
| 64 | Form UX | PARTIAL | Labels, zod, loading, toasts, `beforeunload` guard on 7 | `FieldError` on product/SEO forms (S); in-app nav guard (S); helper text; autosave only as local draft (decision) | 11 | M |
...
| 68 | Search | PARTIAL | Products (whole catalogue) + journal category in the overlay; `⌘K` in the Studio | Collections/portfolio/materials/pages not searched (M, behaviour change under HARD RULE 5 → decision) | 5 | M |
...
| 72 | Security | EXISTS | `requireStaff` re-validation, SSRF guard, form tokens, rate limits, last-admin protection | CSP report-only (own PR); RR-002/003 from QA/03 still open | 16 | M |
| 73 | Database safety | EXISTS | 44 additive migrations, unpooled URL for migrate, preflight | Safe seed strategy = D8 | 15 | n/a |
| 74 | Demo data seeding | DECISION | No `seed:demo` | D8 design 3; idempotency keys exist | 15 | M |
| 75 | Demo data quality | DECISION | — | Fixture matrix under D8 | 15 | S |
| 76 | Testimonial edge cases | DATA_GAP | Needs §17 columns | After Phase 9 | 15 | S |
...
| 81 | Session checkpoint system | PARTIAL | Narrative `PROJECT_STATE.md` with equivalents | Checkpoint block added at the top in this PR; contradictions corrected | 0 | S |
| 82 | Git strategy | DECISION | Feature branch per phase, draft PR, owner merges; this session on `claude/session-6h1a70` | Keep the existing convention; `redesign/*` names not adopted | — | n/a |
| 83 | Implementation principle | PROCESS | Applied in §3, §5, §10 | — | — | n/a |
| 84 | Component inspiration rule | PROCESS | Adapt-only policy §14 | — | — | n/a |
...
| 88 | Final definition of done | EXISTS | Every gate exists and is wired in `ci.yml`, which executes (run #71 on PR #29) | Retire D4's premise (D27); keep local runs for detail routes, E2E and extra widths | every phase | n/a |
| 89 | First action | PROCESS | This document and the roadmap | STOP after Phase 0 | 0 | n/a |

**Counts:** EXISTS 19 · PARTIAL 52 · DATA_GAP 4 · DECISION 9 · MISSING 1 · PROCESS 4. Forbidden asks: 18 (§4.3); forbidden library pattern families: 20 (§14).
