...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A1 | P1 | Storefront | `/` (worst), all public routes | TBT 2.2–2.9 s, LCP 4.5 s on mobile profile | BASELINE.md: scriptEvaluation 4.56 s + styleLayout 3.5 s of 10.8 s main-thread | Reveal was the ONLY unguarded scroll effect — every sibling already bailed on coarse pointers before its GSAP import; Reveal loaded GSAP + ScrollTriggers on mobile for every instance | `motion/reveal.tsx` | **Landed (PR-6):** Reveal now bails on touch before the import, on the codebase's own `useIsTouch` guard — zero motion bytes / zero ScrollTriggers on touch, desktop unchanged. After-measurement handed to the owner (preview unreachable from the audit environment) — RESULTS.md | Low — content renders fully visible, the reduced-motion path |
...
| Q2 | P2 | Studio | `/studio/media` | 2,854/2,854 media-library files have no alt text; inherited everywhere used | Content Health screen | Backlog from bulk imports | Media library | Assisted alt-writing pass in batches (queued after the Kanban sequence) | Low — per-file owner review anyway |
