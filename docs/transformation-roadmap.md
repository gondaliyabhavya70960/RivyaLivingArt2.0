...
- ~~Sonner styled by tokens; badge semantic tones; a `Tabs` primitive~~ **DONE 2026-09-03**. `ui/tabs.tsx` on Radix (pill + underline variants), consumed by `translations-section.tsx`, whose hand-rolled strip declared `role="tab"` without `aria-controls`, gave the panel no `role="tabpanel"`, and kept all eight locales in the tab order — measured 8 tab stops → 1. Sticky header row: **already done and verified** — only 2 of 21 tables (products, inquiries) carry a `min-w` and both go `xl:sticky`; at 1440 the products `<thead>` pins at 64 px. The other 19 sit in `overflow-x-auto` wrappers that become their own scroll container, so sticky there is a silent no-op. **Pinned first column is the one item still open, and is blocked**: `StudioRow`'s tints are semi-transparent, so a pinned cell lets scrolled-under content through — it needs an opaque `color-mix` composite on the row first. Row-enter for appended activity rows (shortlist 15) also remains.
...
- Library: cursor pagination past 200, sort, date/size/orientation filters, detail drawer (metadata, usages with deep links, replace file), drag-and-drop upload, move between folders, bulk alt edit, grid/list toggle; video poster at ingest is a new step (ffmpeg is not a dependency; decide) or a client-captured frame.
