...
| D7 ✅ **YES (2026-09-04)** — additive, one entity per PR; batch B0's eleven migrations are the precedent | May schema changes proceed as their own additive PRs under this transformation? | 9, 11, 12, 13, 14 |
...
| D14 ✅ **YES (2026-09-04)** — REVIEW and ARCHIVED on `ContentStatus` (migration `20260904101000`), Testimonial's own enum, Faq a `status` column (`20260904103000`); every editor offers the four since 2026-09-07 | `ContentStatus` gains REVIEW and ARCHIVED for all four entities, or only the new Testimonial enum? Faq gets a `published` flag? | 9, 11 |
| D15 ✅ **YES (2026-09-04)** — all ten, one per commit; the catalogue is closed at sixteen by `custom-blocks.test.ts` | Which blocks join the six-type catalogue, one at a time? | 11 |
| D16 ✅ **Registry pattern (2026-09-04)** — `/studio/process` and `/studio/materials` are the sections board pre-filtered, fixed arity kept | Process and Materials stay fixed-arity slots, or become registry-pattern entities? | 8, 11 |
...
- Product form: ~~tabs (General · Images · Customization · Details · SEO)~~ **DONE 2026-09-03** — every panel `forceMount`ed so no field, upload or editor instance unmounts on a tab change, and each tab declares its fields so a refused submit lands on the tab holding the error rather than nowhere. ~~two columns with the draft preview~~ **DONE 2026-09-06** — `EditorSplit` docks `DraftPreviewPanel` (the phone frame, 1:1, keyed on the save count so each save reloads it; Tablet and Desktop open the dialog) beside the product and journal forms when the CONTENT AREA is 64rem or wider — a container query, so it follows the sidebar's collapse rather than picking a viewport width that is wrong for one of its two states; narrower, the footer button and the dialog carry on; the frame mounts only while the column has layout (`useIsDisplayed`) — Chromium loads a hidden lazy iframe eagerly, measured, so lazy alone did not keep a phone from fetching the page. §12.5's unsaved-changes indicator sits in both footers. ~~`FieldError` everywhere, helper text~~ **DONE 2026-09-05** — a `FieldHint` primitive and `describedBy()`; the five `useState` forms (site copy, navigation, research, add-sources, review editor) judge each field in the browser with the rule the action holds (`describeCopyProblem` and a new shared `describeLabelProblem` where a validator exists; the schema's limits mirrored where not), `aria-invalid` + `aria-describedby` on every control, focus on the first refused field. Blog and portfolio editors likewise; ~~in-app navigation interception in `useUnsavedChangesGuard`~~ **DONE 2026-09-03** (it was `beforeunload` only, so seven forms discarded edits on any sidebar click — measured: clean form navigates, dirty form is held, both answers behave) — **two holes measured 2026-09-07**: the command palette navigates with `router.push`, past the click listener, and went with no question (fixed: it hands the destination to the guard's dialog); and Browser Back was never covered despite the hook's `popstate` handler — Next's own listener unmounts the form first, so the handler is removed and **Back stays open (M: a same-URL sentinel entry pushed when the form goes dirty)**; ~~guard on dialog CRUD~~ **DONE 2026-09-03** (they guarded only `while busy` — the save round trip — so a stray Escape before it discarded the typing; `useDismissGuard` covers the dirty case and blocks rather than stacking a dialog on a dialog); ~~`window.prompt` → `Dialog` + `Input`~~ **DONE 2026-09-03** (all three sites; no native prompt fires); ~~`MediaPicker` wired into the editor image button~~ **DONE 2026-09-03** — the picker sits beside the URL button (different jobs: the library, or an image that lives elsewhere); `MediaPicker` gained an optional `trigger` and its five existing consumers are unchanged. ~~Picker gains video~~ **DONE 2026-09-04 by batch D; reach completed 2026-09-07** — `MediaPicker accept="VIDEO"` lists the library's films with poster and duration and batch D wired it into the testimonial's video and the two film blocks; the product's Video URL (the Images tab and the print section that shares the field), the portfolio's Video URL and the site-images board's Hero video slot were still bare address boxes and now carry the same input-plus-picker pair.
- ~~Autosave only as local-draft persistence~~ **DONE 2026-09-03 on the three forms it was scoped to** (verified against HEAD 2026-09-07): `useLocalDraft` debounces every value change into `localStorage["studio:draft:<entity>:<id|new>"]` 800 ms after it — never the database — and `LocalDraftBar` offers Restore · Discard; mounted by `product-form.tsx`, `blog-post-form.tsx` and `portfolio-form.tsx`, discarded on a successful save. Two defects found in that reading and fixed 2026-09-07: Restore did not repaint the journal's rich-text body (Tiptap reads its value once on mount, so the editors are now keyed on the hook's restore count), and the unsaved-changes indicator and the navigation guard stayed armed after a successful save in edit mode (`isDirty` compares against the values the form MOUNTED with; a save now resets that baseline). ~~**Still open, ungated, S each:** the hook is not mounted on the testimonial, landing-page, legal-page, settings and SEO forms (all react-hook-form — the same five lines), and the FAQ, category and research dialogs are `useState` forms that need a value-shaped variant of the hook first.~~ **DONE 2026-09-07 (seventh batch)** — the five editors carry `useLocalDraft` (the testimonial brings its two link pickers along on Restore; the legal pages key their rich-text body and translations on the restore count; settings and SEO use a `singleton` slot), and the three dialogs carry `useLocalDraftValue`, the value-shaped sibling: `values`/`initial`/`apply`, never a draft equal to what the dialog opened with, a mount's own draft removed if the edit is undone, a pre-existing one offered rather than imposed, kept on Cancel, cleared on save. On the way: Restore keeps the mount baseline so a restored edit reads as unsaved (indicator and guard armed); Discard cancels a write still pending in the debounce; the dialog bodies remount per open (Radix unmounts the content, not the body — the create dialogs' typed fields used to survive a Cancel); a deleted row's draft is discarded, from the lists' bulk delete as well as the editors' own; Restore folds a stored draft onto the form's current shape (`coerceDraftValues`), so one written before a field was added or renamed never plants `undefined` in a controlled input, and one with nothing usable in it is discarded by Restore itself; a draft the mount did not write is never written over — it is the person's to Restore or Discard, the bar says autosave waits, and a stray keystroke into an autofocused field no longer replaces a crash's draft 800 ms later; the bar's two buttons are off while a save is in flight; a write still pending when a dialog is cancelled or an editor is left is flushed rather than dropped; the category dialog gets the 85vh ceiling its siblings had (it ran to 1350px with its buttons below a laptop's fold). **Open, S:** the bar cannot tell a draft older than the row's last save from a newer one — surface `updatedAt` on each form's initial and label (or hide) a draft that predates it. **The per-row draft column ("unless … decided") stays an owner decision:** no `draft` column exists on Product, BlogPost or Portfolio — only `SiteCopy.draftValue` and `SiteImage.draft` (migration `20260824060000`) — so a database-backed Save draft that stages without touching the live row needs an additive migration per entity under D7's rule.
- ~~D15 blocks, one per commit: Collection Grid, Portfolio Grid, Journal Grid, Testimonial (after Phase 9); then Video Hero / Video Story / galleries after the picker supports video and `media-usages.ts` walks them; `custom-blocks.test.ts` updated deliberately each time~~ **DONE 2026-09-04 by the content series** (verified against HEAD 2026-09-07): all ten are in `CUSTOM_BLOCK_TYPES` (`custom-blocks.ts:295-304` — collectionGrid, portfolioGrid, journalGrid, testimonial, testimonialGrid, videoHero, videoStory, masonryGallery, bentoGallery, fullscreenGallery), each with an editor branch in `block-fields.tsx`, a renderer in `custom-page-blocks.tsx`, and `media-usages.ts:296-325` walking the film, the poster and every gallery frame; `custom-blocks.test.ts:53` closes the catalogue at sixteen. videoStory and the three galleries sit on the CI-swept demo lander (`prisma/fixtures/demo/custom-blocks.json`), so the design and a11y sweeps render those four; the other six (collectionGrid, portfolioGrid, journalGrid, testimonial, testimonialGrid, videoHero) are covered by the unit suite only — see the found-open line below.
...
- D7 items: ~~Category `seoTitle/seoDescription/visible`~~ **DONE 2026-09-04** (migration `20260904104000_category_seo_visible`; the category editor's SEO title, description and Visible controls; every public category reader selects `visible: true`, and the category page marks a hidden shelf `noindex` and 404s it unless staff draft mode is on); ~~D16 Process/Materials if promoted (registry pattern, D2)~~ **DONE 2026-09-04** — `/studio/process` and `/studio/materials` are the sections board pre-filtered to `process-steps` (×10) and `materials` (×4) over `page-sections.ts`, fixed arity by D16's own terms; ~~D14 statuses~~ **DONE 2026-09-04, one hole closed 2026-09-07** — the enum, the migrations, list tabs and bulk actions on all three entities, every public reader failing closed on exactly PUBLISHED, and the blog and portfolio editors offering all four states; the product editor's status Select offered only Draft and Published, so a row moved to Review or Archived from the list opened with no matching option (it lists the four now). `BlogPost.publishedAt` as a visibility gate **stays a decision**: today a PUBLISHED post with a future `publishedAt` is live at once and sorts to the top of every listing; making it behave like `CustomPage.isLive()` is a small, schema-free change across the public blog readers, but it silently changes which posts visitors see, so it waits for the owner. Residual, minor: the draft-preview pill on the three detail routes keys on `=== "DRAFT"`, so a REVIEW or ARCHIVED row in staff preview shows no banner.
- **Found open in the 2026-09-07 reading** (each S and ungated unless marked): REDESIGN.md §12.5's footer verbs — Discard · Save draft · Publish beside the indicator on the product and journal editors (client-only: `upsertProduct` and `upsertBlogPost` already take `status`, and the footer must be designed against D14's four states); ~~the local-draft hook on the five remaining forms (above)~~ (DONE 2026-09-07); the demo lander fixture opens with a plain `hero` and carries none of `collectionGrid`, `portfolioGrid`, `journalGrid`, `testimonial`, `testimonialGrid` or `videoHero`, so CI's design and a11y sweeps never render those six — a fixture-only change, to be run through the band-rhythm guard; a by-id testimonial reader for the `testimonial` block, which scans a 500-row pool today; §12.5's sortable product columns, which today re-order the 50 rows on screen rather than the catalogue, with Title and Status not sortable at all (a page-query `orderBy`, not a Server Action — M); §12.5's portfolio CMS as a thumbnail grid, which exists only as the phone fallback under a table (M); section arrangements are never revisioned — `publishSections` writes no `ContentRevision`, so the composer's history covers a surface's words and pictures but not its order (a Server Action change, **gated**); the media picker beside the 3D-model field — `listMediaForPicker` lists only IMAGE and VIDEO, and widening that enum is an additive Server Action input change, so it is a decision, not a batch (M, **gated**). **From the adversarial pass over this reading (2026-09-07), fixed the same day:** the product form's tab map now follows the print swap (a bad video or 3D URL on a print product used to switch to a tab that did not hold the field); the testimonial form runs `describeTestimonialProblem` before the action and lands the reason on the Permission control (`runAction` had been turning the refusal into "Something went wrong"); the legal-page form resets its baseline after a save; the three social-image routes select PUBLISHED and spread the demo clause like every other public reader (verifying it showed they are unreachable at any URL — under `[locale]`, exempted and un-prefixed by the proxy — while every page points `og:image` at its cover or slot; wire or delete is **a decision**); the product page treats a hidden category as unset (no crumb, no BreadcrumbList item, no "View collection" to a 404); the Studio audit sweeps the composer's Pictures and Order tabs. **From that pass, ungated:** ~~the react-hook-form editors mirror none of their actions' length caps … so an over-long value is refused server-side as a raw zod message in a toast with no field named (S per form)~~ **DONE 2026-09-07 (eighth batch)** — every cap, range and format the eight actions enforce now lives in `src/lib/studio-limits.ts` and BOTH sides import it, because a `"use server"` file may export only async functions and so cannot hand its numbers to a test; a new drift test walks the thirteen schema declarations across the eight action files and fails on a literal cap. Two corrections to the sentence above, both measured: the symptom is a raw zod message for the five actions that `safeParse` ("Too big: expected string to have <=300 characters"), but `testimonials` and `products` `.parse()` inside `runAction`, which flattens every refusal to "Something went wrong. Please try again." — no message at all; and the caps are NOT uniform in kind, since `settings`, `pages`, `blog`, `custom-pages` and `testimonials` trim before counting while `products` and `portfolio` count the raw string, so a uniform mirror would have disagreed with the server in one direction or the other. Also closed on the way: the two settings editors each submit the other's fields and the action reports only its first issue in schema key order, so on a row with an empty WhatsApp number every SEO save was refused with a message about a field on another page — both now check the half they do not render and name the screen; and the testimonial's four media fields mirror the host rule rather than failing as "Something went wrong"; the category's `seoTitle`/`seoDescription` are not per-locale, unlike the product's and the post's (S: two `localize.ts` entries, two translation fields, one page read); a unit test for the composer's surface↔page join, with `SURFACE_PREVIEW_PATH` moved into `src/lib/surface-tabs.ts` (S); audit row 64's "inline `role=status` success" is undecided — every save success is a toast today. **Two decisions:** the product's and the portfolio's `optionalUrl` — the form schema and the Server Action alike — accept only absolute URLs, while the library the new picker reads can hold site-root paths (`/uploads/…` on credential-less storage, `/media/v3/…` in the demo set; production Blob URLs are `https://` and pass), so a pick from those rows is refused at Save with "Enter a valid URL." — the testimonial's `urlFieldSchema` already accepts a leading `/`, and widening the two validators to match is additive on both sides, but the action half is a Server Action input change; and `setProductsStatus` (the list's bulk bar) writes `status` without stamping `ownerTouched`, so the deploy-time sheet fill can re-publish a sheet-imported product the owner archived or sent to review — the fix is one stamp inside a Server Action.

Touches: `src/components/studio/{pages,products,blog,portfolio,custom-pages,publish}/*`, `src/lib/custom-blocks.ts`, `custom-page-blocks.tsx`, `src/hooks/use-unsaved-changes-guard.ts`, `rich-text-editor.tsx`, `media-picker.tsx`, `prisma/` (per decision). Effort: L. Gates: D7, D14, D15, D16 — all four answered 2026-09-04 (batch B0); what the phase still owes is ungated except where a line says otherwise.
