...
- ~~Studio `error.tsx` with a reference code; `not-found.tsx` in v3 vocabulary; the shared `loading.tsx` flat, no shimmer~~ **DONE 2026-09-03** — all three retokened and driven against a production build (the audit sweeps 30 routes and none of them is an error). **Per-route skeletons DONE 2026-09-03** — twelve of them, composed from a new `studio/skeleton.tsx` (header footprint, filter bar, table at real row height, media grid), flat and at final dimensions. Still open here: the auth-tree boundary.
- ~~Sonner styled by tokens; badge semantic tones~~ **DONE 2026-09-03**. Sticky header row: **already done and verified** — only 2 of 21 tables (products, inquiries) carry a `min-w` and both go `xl:sticky`; at 1440 the products `<thead>` pins at 64 px under the topbar. The other 19 sit in `overflow-x-auto` wrappers that become their own scroll container, so sticky there would be a silent no-op. **Pinned first column is still open and blocked on one thing**: `StudioRow`'s tints are semi-transparent, so a pinned cell lets scrolled-under content through — it needs an opaque `color-mix` composite on the row first. Still open too: a `Tabs` primitive (`translations-section.tsx` hand-rolls one and is the real consumer); row-enter for appended activity rows (shortlist 15).
